...
V10_H4_SUPERNODE_FACTOR_INTEGRITY_RETAINED_LIMIT_GIB = 30.0
...
def _run_worker(
    ...
    *,
    ...
    formal_v9_layer_supernode = (
        getattr(plan, "method", "") == V9_H4_LAYER_SUPERNODE_METHOD
    )
    ...
    formal_v10_side_packet_full_producer = (
        getattr(plan, "method", "") == V10_H4_SIDE_RESPONSE_PACKET_FULL_PRODUCER_METHOD
    )
    ...
    if formal_v9_bare_f_side:
        ...
        terminate_limit = float(absolute_terminate_memory_bytes)
    if formal_v9_layer_supernode:
        absolute_terminate_memory_bytes = V9_H4_LAYER_SUPERNODE_HARD_STOP_BYTES
        terminate_limit = float(absolute_terminate_memory_bytes)
    if formal_v10_factor_integrity:
        absolute_terminate_memory_bytes = (
    ...
    if formal_v10_side_packet_consumer:
        ...
        terminate_limit = float(absolute_terminate_memory_bytes)
    if formal_v10_side_packet_full_producer:
        absolute_terminate_memory_bytes = (
            V10_H4_SIDE_RESPONSE_PACKET_FULL_PRODUCER_HARD_STOP_BYTES
        )
        terminate_limit = float(absolute_terminate_memory_bytes)
    if formal_v10_side_packet_compression:
        absolute_terminate_memory_bytes = (
    ...
    if formal_v9_layer_supernode:
        ...
        for method_name in methods:
            method_intervals[method_name] = {
                ...
                "construction": _v5_h4_blr_candidate_interval_peak(
                    ...
                    formal_samples_path,
                    "bottom",
                    begin_stage=(f"v9_layer_supernode_bottom_{method_name}_begin"),
                    end_stage=f"v9_layer_supernode_bottom_{method_name}_cleanup",
                    limit_gib=V9_H4_LAYER_SUPERNODE_CONSTRUCTION_LIMIT_GIB,
                ),
            }
        overall_construction_interval = _v5_h4_blr_candidate_interval_peak(
            formal_stages_path,
            formal_samples_path,
            "bottom",
            begin_stage="v9_layer_supernode_bottom_construction_begin",
            end_stage="v9_layer_supernode_bottom_construction_end",
            limit_gib=V9_H4_LAYER_SUPERNODE_CONSTRUCTION_LIMIT_GIB,
        )
        overall_retained_interval = _v5_h4_blr_candidate_interval_peak(
            formal_stages_path,
            formal_samples_path,
            "bottom",
            begin_stage="v9_layer_supernode_bottom_retained_apply_state_ready",
            end_stage="v9_layer_supernode_bottom_retained_state_release",
            limit_gib=V9_H4_LAYER_SUPERNODE_RETAINED_LIMIT_GIB,
        )
        construction_pass = (
            overall_construction_interval.get("status") == "measured"
            and overall_construction_interval.get("pass") is True
        ...
        worker_stability_gate: bool | None = None
        worker_record_status = "not_available"
        ...
        if worker_record_path.is_file():
            try:
                ...
                identity_matches = (
                    isinstance(worker_record, Mapping)
                    and worker_record.get("schema") == V9_H4_LAYER_SUPERNODE_SCHEMA
                    and worker_record.get("method") == V9_H4_LAYER_SUPERNODE_METHOD
                    and worker_record.get("source_sha")
                    == getattr(plan, "source_sha", None)
                )
                if not identity_matches:
                    ...
                    worker_gate = worker_record.get("gate")
                    value = (
                        worker_gate.get("numerical_stability_gate_pass")
                        if isinstance(worker_gate, Mapping)
                        else None
                    )
                    ...
                    stable_record_contract_matches = value is not True or (
                        worker_record.get("preferred_method") in ("SN2-J", "SN2-SGS")
                    ...
                    if (
                        ...
                        and stable_record_contract_matches
                    ):
                        worker_stability_gate = value
                        ...
                        worker_record_status = "contract_mismatch"
                        worker_contract_reason = (
                            "stability_gate_field_missing_or_invalid"
                        )
        swap_pass = sample_count > 0 and zero_swap_observed is True and peak_swap == 0
        construction_measured = (
            overall_construction_interval.get("status") == "measured"
        )
        retained_measured = overall_retained_interval.get("status") == "measured"
        construction_failed = construction_measured and (
            overall_construction_interval.get("pass") is False
        )
        retained_failed = retained_measured and (
            overall_retained_interval.get("pass") is False
        )
        if worker_stability_gate is False and not retained_measured:
            overall_status = "numerical_stability_failed_retained_not_run"
            overall_pass = False
        elif worker_stability_gate is False:
            overall_status = "numerical_stability_failed"
            overall_pass = False
        elif worker_stability_gate is None:
            overall_status = (
                "contract_mismatch"
            ...
            overall_pass = None
        elif construction_failed or retained_failed:
            ...
            overall_status = "pass"
            overall_pass = True
        resource_authority["v9_h4_layer_supernode_bottom_telemetry"] = {
            "raw_marker_path": str(formal_markers_path),
            "process_tree_samples_path": str(formal_samples_path),
            "memory_stages_path": str(formal_stages_path),
            "memory_object_ledger_path": str(formal_object_ledger_path),
            ...
            "profile": V9_H4_LAYER_SUPERNODE_SCHEMA,
            "method_intervals": method_intervals,
            "method_interval_role": (
                "evidence_only_checkpoint; not a substitute for overall intervals"
            ),
            ...
            "numerical_stability_gate_pass": worker_stability_gate,
            "overall": {
                "status": overall_status,
                "pass": overall_pass,
                "construction_pass": construction_pass,
            ...
            "gate_contract": {
                ...
                "stability_gate": "finite_repeat_linearity_only",
            ...
            "absolute_terminate_memory_bytes": (V9_H4_LAYER_SUPERNODE_HARD_STOP_BYTES),
            "effective_hard_stop_memory_gib": (
                V9_H4_LAYER_SUPERNODE_HARD_STOP_BYTES / 1024**3
            ),
            "require_zero_swap": True,
            "zero_swap_observed": zero_swap_observed if sample_count else None,
            "overall_process_tree_peak_bytes": peak_process_tree,
    ...
    if formal_v10_factor_integrity:
        construction_interval = _v5_h4_blr_candidate_interval_peak(
            ...
            formal_samples_path,
            "bottom",
            begin_stage="v10_layer_supernode_bottom_construction_begin",
            end_stage="v10_layer_supernode_bottom_construction_end",
            limit_gib=V10_H4_SUPERNODE_FACTOR_INTEGRITY_CONSTRUCTION_LIMIT_GIB,
        )
        retained_interval = {
            "status": "not_applicable",
            "peak_process_tree_rss_gib": None,
            "limit_gib": V10_H4_SUPERNODE_FACTOR_INTEGRITY_RETAINED_LIMIT_GIB,
            "pass": None,
            "reason": "v10_factor_integrity_has_no_retained_candidate",
        }
        worker_record_path = (
            run_directory / "numerical_output" / "v3_v7_diagnostic.json"
        )
        worker_numerical_gate: bool | None = None
        worker_numerical_classification: str | None = None
        worker_record_status = "not_available"
        worker_contract_valid = False
        worker_contract_reason = "not_checked"
        if worker_record_path.is_file():
            try:
                ...
                if not identity_matches:
                    ...
                    worker_gate = worker_record.get("gate")
                    worker_numerical_classification = worker_record.get(
                        "classification"
                    )
                    value = (
                    ...
                    allowed_classifications = {
                        ...
                        "FACTOR_ONLY_DETACH_IMPLEMENTATION_FAILURE",
                        "SUPERNODE_SCATTER_LAYOUT_FAILURE",
                        "SUPERNODE_ACTION_WORKSPACE_FAILURE",
                    }
                    classification_contract_matches = (
                        worker_numerical_classification in allowed_classifications
                        and value
                        == (
                            worker_numerical_classification
                            == "SUPERNODE_FACTOR_INTEGRITY_PASS"
                        )
                    )
                    exact_spool_contract_matches = (
                        worker_record.get("exact_spool_opened") is True
                    )
                    if (
                        isinstance(value, bool)
                        and factor_contract_matches
                        and packet_contract_matches
                        and classification_contract_matches
                        and exact_spool_contract_matches
                    ):
                        worker_numerical_gate = value
                        worker_record_status = "measured"
                        worker_contract_valid = True
                        ...
                        worker_record_status = "contract_mismatch"
                        worker_contract_reason = "factor_packet_classification_or_numerical_gate_contract_mismatch"
        construction_pass = (
            construction_interval.get("status") == "measured"
            and construction_interval.get("pass") is True
        )
        ...
        if worker_numerical_gate is False:
            ...
            overall_status = (
                ...
                else "resource_interval_not_available"
            )
            overall_pass = (
                False if construction_interval.get("status") == "measured" else None
            )
        elif not swap_pass:
            overall_status = "swap_gate_failed"
            overall_pass = False
            ...
            overall_pass = True
        resource_authority["v10_h4_supernode_factor_integrity_telemetry"] = {
            "raw_marker_path": str(formal_markers_path),
            "process_tree_samples_path": str(formal_samples_path),
            "memory_stages_path": str(formal_stages_path),
            "memory_object_ledger_path": str(formal_object_ledger_path),
            ...
            "profile": V10_H4_SUPERNODE_FACTOR_INTEGRITY_SCHEMA,
            "construction_interval_summary": construction_interval,
            "retained_interval_summary": retained_interval,
            "worker_record_path": str(worker_record_path),
            "worker_record_status": worker_record_status,
            ...
            "overall": {
                ...
                "numerical_gate_pass": worker_numerical_gate,
                "swap_pass": swap_pass,
                "resource_gate": ("construction_and_swap_pass_retained_not_applicable"),
            },
            "gate_contract": {
                "construction_peak_limit_gib": (
                    V10_H4_SUPERNODE_FACTOR_INTEGRITY_CONSTRUCTION_LIMIT_GIB
                ),
                "retained_peak_limit_gib": (
                    V10_H4_SUPERNODE_FACTOR_INTEGRITY_RETAINED_LIMIT_GIB
                ),
    ...
    if formal_v10_j1_inner_fgmres:
        construction_interval = _v5_h4_blr_candidate_interval_peak(
            ...
            formal_samples_path,
            "bottom",
            begin_stage="v10_j1_inner_fgmres_bottom_construction_begin",
            end_stage="v10_j1_inner_fgmres_bottom_construction_end",
            limit_gib=V10_H4_J1_INNER_FGMRES_CONSTRUCTION_LIMIT_GIB,
        )
        retained_interval = _v5_h4_blr_candidate_interval_peak(
            formal_stages_path,
            formal_samples_path,
            "bottom",
            begin_stage="v10_j1_inner_fgmres_bottom_retained_apply_state_ready",
            end_stage="v10_j1_inner_fgmres_bottom_retained_state_release",
            limit_gib=V10_H4_J1_INNER_FGMRES_RETAINED_LIMIT_GIB,
        )
        worker_record_path = (
            run_directory / "numerical_output" / "v3_v7_diagnostic.json"
        )
        ...
        if worker_record_path.is_file():
            try:
                ...
                identity_matches = (
                    isinstance(worker_record, Mapping)
                    and worker_record.get("schema") == V10_H4_J1_INNER_FGMRES_SCHEMA
                    and worker_record.get("method") == V10_H4_J1_INNER_FGMRES_METHOD
                    and worker_record.get("source_sha")
                    == getattr(plan, "source_sha", None)
                )
                if not identity_matches:
                    ...
                    factor_contract_matches = (
                        isinstance(factor_inventory, Mapping)
                        and factor_inventory.get("layer_factor_count_ready") == 6
                        and factor_inventory.get("layer_factor_count_after_cleanup")
                        == 0
                        and factor_inventory.get("full_side_exact_factor_count") == 0
                        and factor_inventory.get("global_direct_factor_count") == 0
                        and factor_inventory.get("nested_ksp_count") == 0
                        and factor_inventory.get("side_fgmres_ksp_count") == 5
                        and factor_inventory.get("pc_nested_ksp_count") == 0
                    )
                    packet_contract_matches = (
                        worker_record.get("selected_mode_packet_opened") is False
                        and worker_record.get("qep_count") == 0
                    ...
                    lifecycle_contract_matches = (
                        worker_record.get("explicit_components_released_before_krylov")
                        is True
                        and isinstance(worker_record.get("lifecycle"), Mapping)
                        and worker_record["lifecycle"].get("retained_state")
                        == "released_after_five_rhs"
                        and worker_record["lifecycle"].get("system_released") is True
                    )
                    worker_gate = worker_record.get("gate")
                    value = (
                        worker_gate.get("numerical_gate_pass")
                    ...
                    worker_numerical_classification = (
                        worker_gate.get("classification")
                        if isinstance(worker_gate, Mapping)
                        else None
                    )
                    classification_contract_matches = (
                        ...
                        in {
                            ...
                            "J1_INNER_FGMRES_NUMERICAL_LIMIT_NOT_REACHED_BY_32",
                        }
                        and value
                        == (
                            worker_numerical_classification
                    ...
                    if (
                        ...
                        and factor_contract_matches
                        and packet_contract_matches
                        and lifecycle_contract_matches
                        and classification_contract_matches
                    ):
                        worker_numerical_gate = value
                        worker_record_status = "measured"
                        ...
                        worker_record_status = "contract_mismatch"
                        worker_contract_reason = "factor_packet_lifecycle_or_numerical_gate_contract_mismatch"
        ...
        retained_pass = retained_measured and retained_interval.get("pass") is True
        swap_pass = sample_count > 0 and zero_swap_observed is True and peak_swap == 0
        if not construction_measured or not retained_measured:
            resource_gate_summary = "not_available"
        elif construction_pass and retained_pass:
            resource_gate_summary = "construction_and_retained_pass"
        else:
            resource_gate_summary = "construction_or_retained_failed"
        if worker_numerical_gate is None:
            ...
            overall_pass = None
        elif not construction_measured or not retained_measured:
            overall_status = "resource_interval_not_available"
            overall_pass = None
        elif not construction_pass or not retained_pass:
            overall_status = "resource_gate_failed"
            ...
            overall_status = "pass"
            overall_pass = True
        resource_authority["v10_h4_j1_inner_fgmres_telemetry"] = {
            "raw_marker_path": str(formal_markers_path),
            "process_tree_samples_path": str(formal_samples_path),
            "memory_stages_path": str(formal_stages_path),
            "memory_object_ledger_path": str(formal_object_ledger_path),
            ...
            "profile": V10_H4_J1_INNER_FGMRES_SCHEMA,
            "construction_interval_summary": construction_interval,
            "retained_interval_summary": retained_interval,
            "worker_record_path": str(worker_record_path),
            "worker_record_status": worker_record_status,
            ...
            "gate_contract": {
                "construction_peak_limit_gib": (
                    V10_H4_J1_INNER_FGMRES_CONSTRUCTION_LIMIT_GIB
                ),
    ...
    if formal_v10_side_packet_full_producer or formal_v10_side_packet_compression:
        full_producer = formal_v10_side_packet_full_producer
        method_name = (
            V10_H4_SIDE_RESPONSE_PACKET_FULL_PRODUCER_METHOD
            if full_producer
            else V10_H4_SIDE_RESPONSE_PACKET_COMPRESSION_METHOD
        )
        schema_name = (
            V10_H4_SIDE_RESPONSE_PACKET_FULL_PRODUCER_SCHEMA
            if full_producer
            else V10_H4_SIDE_RESPONSE_PACKET_COMPRESSION_SCHEMA
        )
        begin_stage = (
            "v10_side_response_packet_full_producer_begin"
            if full_producer
            else "v10_side_response_packet_compression_begin"
        )
        end_stage = (
            "v10_side_response_packet_full_producer_cleanup"
            if full_producer
            else "v10_side_response_packet_compression_released"
        )
        limit_gib = 60.0 if full_producer else 30.0
        interval = _v5_h4_blr_candidate_interval_peak(
            formal_stages_path,
            formal_samples_path,
            "bottom",
        ...
        if worker_record_path.is_file():
            try:
                ...
                identity_matches = (
                    isinstance(worker_record, Mapping)
                    and worker_record.get("schema") == schema_name
                    and worker_record.get("method") == method_name
                    and worker_record.get("source_sha")
                    == getattr(plan, "source_sha", None)
                )
                inventory = worker_record.get("factor_inventory")
                if full_producer:
                    lifecycle_contract = (
                        isinstance(inventory, Mapping)
                        and inventory.get("exact_side_factor_count_ready") == 1
                        and inventory.get("factor_count_after_cleanup") == 0
                        and inventory.get("global_direct_factor_count") == 0
                        and worker_record.get("selected_mode_packet_opened") is True
                        and worker_record.get("exact_spool_opened") is True
                        and worker_record.get("qep_count") == 0
                    )
                    worker_gate = worker_record.get("full_packet_eligibility_pass")
                else:
                    packet = worker_record.get("packet")
                    lifecycle_contract = (
                        isinstance(inventory, Mapping)
                        and inventory.get("consumer_factor_count") == 0
                        and inventory.get("global_direct_factor_count") == 0
                        and inventory.get("nested_ksp_count") == 0
                        and worker_record.get("selected_mode_packet_opened") is False
                        and worker_record.get("exact_spool_opened") is False
                        and worker_record.get("qep_count") == 0
                        and worker_record.get("sgs_executed") is False
                        and isinstance(packet, Mapping)
                        and packet.get("released") is True
                ...
                if (
                    ...
                    and (not full_producer or isinstance(worker_gate, bool))
                ):
                    worker_record_status = "measured"
                    worker_contract_valid = True
                    worker_contract_reason = "validated"
                    ...
                    worker_record_status = "contract_mismatch"
                    worker_contract_reason = "identity_factor_packet_or_gate_mismatch"
        interval_measured = interval.get("status") == "measured"
        interval_pass = interval_measured and interval.get("pass") is True
        swap_pass = sample_count > 0 and zero_swap_observed is True and peak_swap == 0
        if not worker_contract_valid:
            ...
            overall_status = "swap_gate_failed"
            overall_pass = False
        elif full_producer and worker_gate is False:
            overall_status = "full_packet_eligibility_failed"
            overall_pass = False
        else:
            overall_status = (
                "pass_full_producer_component"
                if full_producer
                else "pass_compression_component"
            )
            overall_pass = True
        resource_authority[
            ...
            "construction_interval_summary": (
                interval if full_producer else {"status": "not_applicable"}
            ),
            "retained_interval_summary": (
                {"status": "not_applicable"} if full_producer else interval
            ),
            "worker_record_path": str(worker_record_path),
            "worker_record_status": worker_record_status,
            "worker_record_contract_valid": worker_contract_valid,
            "worker_record_contract_reason": worker_contract_reason,
            "full_packet_eligibility_pass": worker_gate if full_producer else None,
            "overall": {
                "status": overall_status,
                "pass": overall_pass,
                "construction_pass": interval_pass if full_producer else None,
                "retained_pass": None if full_producer else interval_pass,
                "swap_pass": swap_pass,
                "full_packet_eligibility_pass": worker_gate if full_producer else None,
                "resource_gate": (
                    "construction_pass_retained_not_applicable"
                    if full_producer and interval_pass
                    else "retained_interval_pass"
                    if not full_producer and interval_pass
                    else "construction_or_retained_failed_or_not_available"
                ),
            },
            "gate_contract": {
                "construction_peak_limit_gib": 60.0 if full_producer else None,
                "retained_peak_limit_gib": None if full_producer else 30.0,
                "retained_state": "not_applicable" if full_producer else "measured",
                "swap_required": 0,
                "exact_side_factor_count_ready": 1 if full_producer else 0,
                "factor_count_after_cleanup": 0,
                "consumer_factor_count": 0,
                "global_direct_factor_count": 0,
                "nested_ksp_count": 0,
                "selected_mode_packet_opened": full_producer,
                "exact_spool_opened": full_producer,
                "qep_count": 0,
            },
            "absolute_terminate_memory_bytes": (
                V10_H4_SIDE_RESPONSE_PACKET_FULL_PRODUCER_HARD_STOP_BYTES
                if full_producer
                else V10_H4_SIDE_RESPONSE_PACKET_COMPRESSION_HARD_STOP_BYTES
            ),
            "effective_hard_stop_memory_gib": limit_gib,
            "zero_swap_observed": zero_swap_observed if sample_count else None,
            "overall_process_tree_peak_bytes": peak_process_tree,
            "overall_process_tree_peak_gib": peak_process_tree / 1024**3,
            "overall_peak_swap_bytes": peak_swap,
