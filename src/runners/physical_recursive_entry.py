...
def worker(args):
    ...
    contract=component_contract(args.target);root=Path(args.output)
...
def main():
    ...
    remaining=min(budget['G0_G1_remaining'],budget['batch_remaining'])
    if remaining<=0:raise RuntimeError('G1 compute budget exhausted')
    lock=budget_path.parent/'g1_active.lock'
    ...
    try:
        ...
        for key in ('G0_G1_remaining','batch_remaining'):budget[key]-=charge
