...
def _validate_cross_fields(config: Mapping[str, Any]) -> None:
    ...
    if dimension == 2:
        ...
        if kind == "full3d_iterative":
            ...
            if preconditioner == "full3d_scalable_v1":
            ...
            elif preconditioner == "physical_p4_blr_tradeoff_v17":
                stage = solver.get("stage")
                stage_budgets = {
                    "T1_BLR_CONTROL": (14400, 14400),
                    "T2_BLR_CONTROL": (14400, 14400),
                    "T3_ORIGINAL": (14400, 10800),
                    "T4_NOTCH": (14400, 10800),
                    "T5_FINALIZE": (43200, 43200),
                }
                if stage not in stage_budgets:
                    raise _error(
                        "solver.stage",
                        "physical_p4_blr_tradeoff_v17 requires an explicit T1-T5 stage",
                    )
                for section, key, actual, expected in (
                    ("solver", "restart", solver["restart"], 32),
                    ("solver", "max_iterations", solver["max_iterations"], 2048),
                ...
                if solver.get("memory_policy") != "SYMBOLIC_SIZED_LOCAL_MUMPS_V11":
                    raise _error(
                        "solver.memory_policy",
                        "physical_p4_blr_tradeoff_v17 fixes SYMBOLIC_SIZED_LOCAL_MUMPS_V11",
                    )
                notch = geometry.get("cell_notch")
                if stage == "T4_NOTCH":
                    if notch != "positive_x_middle_y_z40_80":
                        raise _error(
                            "geometry.cell_notch",
                            "T4_NOTCH requires the frozen positive_x_middle_y_z40_80 recipe",
                        )
                elif notch is not None:
                    raise _error(
                        "geometry.cell_notch",
            ...
            elif preconditioner == "physical_p6_trace_p4_condensed_lowmem_v20":
                for section, key, actual, expected in (
                    ("solver", "stage", solver.get("stage"), "Y3_ORIGINAL"),
                    ("solver", "restart", solver["restart"], 32),
                    ("solver", "max_iterations", solver["max_iterations"], 2048),
                    ("solver", "outer_restart", solver.get("outer_restart"), 0),
                    ("solver", "memory_policy", solver.get("memory_policy"), "SYMBOLIC_SIZED_LOCAL_MUMPS_V11"),
                ...
                stage = solver.get("stage")
                stage_budgets = {
                    "Q4_ORIGINAL": (43200, 43200),
                    "Q3_ORIGINAL": (43200, 43200),
                    "Q2_ORIGINAL": (43200, 43200),
                }
                if stage not in stage_budgets:
                    raise _error(
                        "solver.stage",
                ...
                expected_coarse_degree = {
                    "Q4_ORIGINAL": 4,
                    "Q3_ORIGINAL": 3,
                    "Q2_ORIGINAL": 2,
                }[stage]
                if solver.get("coarse_degree") != expected_coarse_degree:
                    raise _error(
                        "solver.coarse_degree",
                        f"{preconditioner} fixes coarse_degree={expected_coarse_degree} for {stage}",
                    )
                for key, expected in (
                    (
                ...
                if geometry.get("model_variant") != "original":
                    raise _error(
                        "geometry.model_variant",
                        f"{preconditioner} requires the original B model",
                    )
                if geometry.get("cell_notch") is not None:
                    raise _error(
                        "geometry.cell_notch",
                        f"{preconditioner} does not allow a notch",
                    )
                try:
                    from src.geometry.v21_frozen_plan import validate_v21_input
                ...
                stage = solver.get("stage")
                stage_budgets = {
                    "U0_PREFLIGHT": (600, 600),
                    "U1_CONTROL_BRIDGE": (1800, 1800),
                    "U2_EXACT_CONTROL": (43200, 43200),
                    "U3_BLR_CONTROL": (43200, 43200),
                    "U4_ORIGINAL": (43200, 43200),
                    "U4_EXACT_FALLBACK": (43200, 43200),
                    "U5_NOTCH": (43200, 43200),
                    "U6_FINALIZE": (43200, 43200),
                }
                if stage not in stage_budgets:
                    raise _error(
                        "solver.stage",
                        f"{preconditioner} requires an explicit U0-U6 stage",
                    )
                for section, key, actual, expected in (
                    ("solver", "restart", solver["restart"], 32),
                    ("solver", "max_iterations", solver["max_iterations"], 2048),
                ...
                if solver.get("memory_policy") != "SYMBOLIC_SIZED_LOCAL_MUMPS_V11":
                    raise _error(
                        "solver.memory_policy",
                        f"{preconditioner} fixes SYMBOLIC_SIZED_LOCAL_MUMPS_V11",
                    )
                notch = geometry.get("cell_notch")
                if stage == "U5_NOTCH":
                    if notch != "positive_x_middle_y_z40_80":
                        raise _error(
                            "geometry.cell_notch",
                            "U5_NOTCH requires the frozen positive_x_middle_y_z40_80 recipe",
                        )
                elif notch is not None:
                    raise _error(
                        "geometry.cell_notch",
                ...
                for section, key, actual, expected in (
                    ...
                    ("execution", "mpi_size", execution["mpi_size"], 1),
                    ("execution", "timeout_seconds", execution["timeout_seconds"], 14400 if preconditioner in ("balanced_h6_recursive_p4_lo_v6", "balanced_h6_recursive_p4_hi_v6", "bounded_entity16_v7", "bounded_projected_seq2_16_v7", "balanced_h6_entity_gcrot8_v8", "balanced_h6_entity_gcrot8_new16_v9") else 10800 if expanded else 7200),
                    ("execution", "require_zero_swap", execution["require_zero_swap"], True),
                    ("discretization", "nedelec_degree", discretization["nedelec_degree"], 6),
                    ("discretization", "mesh_target_nm", discretization["mesh_target_nm"], 10.0),
                ):
...
def _build_3d_config(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if config["method"]["kind"] == "full3d_iterative":
        ...
        if config["solver"]["preconditioner"] in PROFILES:
            result["physical_intermediate_profile"] = profile_facts(config['solver']['preconditioner'])
