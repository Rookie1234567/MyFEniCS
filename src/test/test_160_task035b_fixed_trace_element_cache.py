...
class Task035bFixedTraceElementCacheTests(unittest.TestCase):
    def test_public_persistent_fixed_trace_cold_and_warm_paths(self) -> None:
        with _shared_temporary_directory() as directory:
            cold, cold_audit = persistent_fixed_trace_hcurl_ufl_element(
                2,
                3,
            ...
            warm, warm_audit = persistent_fixed_trace_hcurl_ufl_element(
                2,
                3,
                cache_directory=str(directory),
                source_sha=SOURCE_SHA,
            ...
            self.assertEqual(cold.basix_hash(), warm.basix_hash())
            self.assertEqual(cold.dim, warm.dim)
