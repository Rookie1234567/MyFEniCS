...
def test_predictions_are_monotone_and_never_claim_pde() -> None:
    ...
    assert fine["predicted_total_gib"] > coarse["predicted_total_gib"]
...
def test_budget_classification_is_fail_closed() -> None:
    assert _classification(300.0, 100.0, 256.0) == "infeasible_current_layout"
    assert _classification(200.0, 100.0, 256.0) == "candidate_high_risk"
    assert _classification(100.0, 100.0, 256.0) == "feasible_with_30pct_guardband"
