...
def test_w10a_streaming_projection_and_q_removed_actionable_formula(tmp_path):
    ...
    assert first["audit"] == {
        ...
        "row_block": 3,
        "block_count": 2,
        "gram_column_read_count": 8,
        "direct_projection_pass_count": 2,
        "basis_pass_count": 3,
        "column_read_count": 24,
        "direct_column_read_count": 16,
        "direct_projection_block_count": 2,
        "mmap": True,
        ...
        "gram_bytes": 4 * 4 * 16,
        "explicit_direct_vector_temp_bytes": 2 * 3 * 16,
        "explicit_direct_vector_temp_scope": "one projected/remainder vector per residual and row block; conjugate and BLAS temporaries excluded",
    }
    assert np.array_equal(first["gram"], second["gram"])
    assert all(np.array_equal(first["h"][name], second["h"][name]) for name in residuals)
    assert first["audit"]["basis_pass_count"] == 3
    assert first["audit"]["direct_projection_pass_count"] == 2
    assert all(
        first["measurements"][name]["direct_repeat_exact"] is True
        and first["measurements"][name]["rho_full"] == first["measurements"][name]["direct_repeat_rho_full"]
...
def test_w10a_rank_file_layout_and_non_solver_scope_fail_closed(tmp_path, monkeypatch):
    ...
    assert "project_from_gram" not in source
    assert "direct_projection_pass_count" in source
    ...
    invalid_analysis = {
        ...
        "audit": {
            ...
            "direct_column_read_count": 16,
            "column_read_count": 24,
            "explicit_direct_vector_temp_bytes": 96,
        },
    }
    unavailable = {
...
def test_w10a_parser_and_target_gate_do_not_trust_top_level_pass(tmp_path):
    ...
    measurements = {
        "control_w5_iter200": {
            ...
            "rho_full": 0.0,
            "rho_full_energy_derived": 0.0,
            "direct_residual_squared": 0.0,
            "direct_repeat_rho_full": 0.0,
            "direct_repeat_residual_squared": 0.0,
        ...
        "target_w7_cumulative400": {
            ...
            "rho_full": 0.99,
            "rho_full_energy_derived": 0.99,
            "direct_residual_squared": 1.0,
            "direct_repeat_rho_full": 0.99,
            "direct_repeat_residual_squared": 1.0,
            "direct_repeat_exact": True,
            "rho_optimistic": 0.95,
            "pass": True,
        },
...
def test_w10a_direct_projection_is_authority_for_cancellation_fixture(tmp_path):
    path = tmp_path / "v_basis.bin"
    basis = np.zeros((4, 6), dtype=np.complex128)
    basis[0, 0] = 1.0
    basis[1, 1] = 1.0
    basis[2, 2] = 1.0
    basis[3, 3] = 1.0e-8
    mapped = np.memmap(path, dtype=np.complex128, mode="w+", shape=basis.shape, order="C")
    mapped[:] = basis
    mapped.flush()
    del mapped
    residual = basis.T @ np.asarray([0.4 - 0.2j, -0.3 + 0.1j, 0.7 + 0.2j, 1.0 + 0.4j])
    result = span.analyze_v_basis(
        path,
        {"control_w5_iter200": residual, "target_w7_cumulative400": residual},
        rows=6,
        columns=4,
        row_block=3,
    )
    measurement = result["measurements"]["control_w5_iter200"]
    assert measurement["rho_full"] <= 1.0e-10
    assert 1.0e-9 < measurement["rho_full_energy_derived"] < 2.0e-8
    assert measurement["direct_repeat_exact"] is True
    assert measurement["rho_full"] == measurement["direct_repeat_rho_full"]
