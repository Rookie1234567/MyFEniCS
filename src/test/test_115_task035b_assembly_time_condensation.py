...
from src.solvers.hcurl_fixed_trace_element import (
...
class TestTask035bAssemblyTimeCondensation(unittest.TestCase):
    def test_fixed_p5_trace_p6_interior_kernel_condenses_exactly(
        ...
        self.assertEqual(candidate.full_rows, 750)
        self.assertEqual(candidate.trace_rows, 300)
    ...
    @unittest.skipUnless(
        MPI.COMM_WORLD.size in {1, 2, 8},
        "serial/MPI2/MPI8 ordinary full direct regression",
    )
    def test_standard_full_fixed_rectangular_dtn(self) -> None:
        comm = MPI.COMM_WORLD
        cfg = replace(
            target_stage4_config(degree=2, h_nm=100.0),
            case_name=f"task035b_standard_full_smoke_mpi{comm.size}",
        ...
        summary = run_stage4b_block_grating_3d_case(
            cfg,
            Path(f"/tmp/task035b_standard_full_smoke_mpi{comm.size}"),
        )
        ...
        self.assertEqual(summary["matrix_stats"]["matrix_rows"], 882)
        self.assertGreater(summary["matrix_stats"]["matrix_nnz_used"], 0)

    @unittest.skipUnless(
        MPI.COMM_WORLD.size in {1, 2, 8},
        "serial/MPI2/MPI8 end-to-end assembly-time condensation check",
    )
    def test_mpi_end_to_end_fixed_rectangular_dtn(self) -> None:
        ...
        self.assertEqual(
            len(heap_trim["allocator_reported_pages_released_by_rank"]),
            comm.size,
        )
        ...
        self.assertEqual(
            ...
            {1: 48412.0, 2: 48412.0, 8: 48716.0}[comm.size],
        )
        self.assertLess(
            summary["linear_system_relative_residual"],
            1.0e-10,
    ...
    def test_mpi2_end_to_end_fixed_p5_trace_p6_interior(self) -> None:
        ...
        audit = summary["cell_static_condensation"]
        self.assertEqual(
            audit["active_cell_interior_modes"],
            audit["interior_rows"],
        )
        ...
        self.assertEqual(
            summary["num_augmented_rows"],
            summary["num_independent_trace_rows"] + audit["appended_rows"],
        )
        self.assertFalse(audit["full_global_matrix_allocated"])
        self.assertFalse(audit["full_trace_matrix_allocated"])
