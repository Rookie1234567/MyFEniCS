...
P4_P5_DWR_R_MINIMAL_CYCLE2_RECORD = RECORDS / (
    "actual_dwr_r_adaptive_tetra_p4_p5_h50_cycle2_minimal_periodic_closure_mpi8.json"
)
P4_P5_DWR_R_FULL_CYCLE1_RECORD = RECORDS / (
    "actual_dwr_r_adaptive_tetra_p4_p5_h50_cycle1_full_periodic_closure_mpi8.json"
)
P4_P5_DWR_R_THETA07_FULL_CYCLE1_RECORD = RECORDS / (
    "actual_dwr_r_adaptive_tetra_p4_p5_h50_theta0p7_cycle1_full_periodic_closure_mpi8.json"
)
P4_P5_UNIFORM_LEVEL1_RECORD = RECORDS / "actual_uniform_tetra_level1_p4_p5_mpi8.json"
...
class Task035UniformControlRecordTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.p4_p5_dwr_r_minimal_cycle2 = json.loads(
            P4_P5_DWR_R_MINIMAL_CYCLE2_RECORD.read_text(encoding="utf-8")
        )
        cls.p4_p5_dwr_r_full_cycle1 = json.loads(
            P4_P5_DWR_R_FULL_CYCLE1_RECORD.read_text(encoding="utf-8")
        )
        cls.p4_p5_dwr_r_theta07_full_cycle1 = json.loads(
            P4_P5_DWR_R_THETA07_FULL_CYCLE1_RECORD.read_text(encoding="utf-8")
        )
        cls.p4_p5_uniform_level1 = json.loads(
            P4_P5_UNIFORM_LEVEL1_RECORD.read_text(encoding="utf-8")
        )
    ...
    def test_p3_p4_minimal_closure_cycle2_is_cost_positive_precursor(self) -> None:
        minimal = self.p3_p4_dwr_r_minimal_cycle2
        self.assertEqual(minimal["status"], "actual_dwr_adaptive_cycles_pass")
        self.assertTrue(minimal["qualification"]["pass"])
        self.assertEqual(minimal["qualification"]["failures"], [])
        self.assertEqual(
            minimal["source"]["commit_sha"],
            "e1268ee31f07aff18a0866436608e4c9ce01e49f",
        )
        self.assertTrue(minimal["source"]["stable_and_clean_after"])
        self.assertEqual(minimal["dwr_marker_policy"], "R_total")
        self.assertEqual(
            minimal["periodic_edge_closure_policy"],
            "minimal_periodic_mates_only",
        )
        self.assertEqual(
            [cycle["mesh_audit"]["global_cell_count"] for cycle in minimal["cycles"]],
            [180, 960, 1968],
        )
        self.assertEqual(
            [cycle["marker"]["marked_count"] for cycle in minimal["cycles"]],
            [42, 68, 292],
        )
        self.assertEqual(
            [cycle["enriched_fixed_reference_error_l2"] for cycle in minimal["cycles"]],
            [0.3001543392450342, 0.1739617477693137, 0.010745243720423922],
        )
        self.assertTrue(minimal["all_fixed_reference_error_reductions_positive"])
        self.assertTrue(
            all(
                refinement["periodic_edge_closure"][
                    "full_periodic_boundary_synchronization"
                ]
                is False
                and refinement["periodic_edge_closure"][
                    "boundary_sleeve_edges_added"
                ]
                == 0
                for refinement in minimal["refinements"]
            )
        )
        final = minimal["cycles"][-1]
        full = self.p3_p4_dwr_r_cycle2_tie_v1
        full_final = full["cycles"][-1]
        self.assertEqual(final["enriched"]["num_nedelec_dofs"], 84164)
        self.assertLess(
            final["enriched"]["num_nedelec_dofs"],
            0.27 * full_final["enriched"]["num_nedelec_dofs"],
        )
        self.assertGreater(
            final["enriched_fixed_reference_error_l2"],
            20.0 * full_final["enriched_fixed_reference_error_l2"],
        )
        self.assertLess(minimal["elapsed_seconds"], 0.16 * full["elapsed_seconds"])
        self.assertLess(
            minimal["resource_authority"]["memory_authority_gib"],
            0.38 * full["resource_authority"]["memory_authority_gib"],
        )
        points = {entry["key"]: entry for entry in self.case093["points"]}
        reference = points["p4_h5"]["full3d"]["official_values"]
        p4_h10 = points["p4_h10"]["full3d"]
        ...
        self.assertGreater(final["enriched_fixed_reference_error_l2"], p4_h10_error)
        self.assertGreater(
            final["enriched"]["num_nedelec_dofs"],
    ...
    def test_p4_p5_minimal_cycle2_is_accurate_but_cost_limited(self) -> None:
        record = self.p4_p5_dwr_r_minimal_cycle2
        self.assertEqual(record["status"], "actual_dwr_adaptive_cycles_pass")
        self.assertTrue(record["qualification"]["pass"])
        self.assertEqual(record["qualification"]["failures"], [])
        self.assertEqual(
            record["source"]["commit_sha"],
            "f3b38cc15359e22ae1548ed40b838f14caefaf3e",
        )
        self.assertTrue(record["source"]["stable_and_clean_after"])
        self.assertEqual(record["periodic_edge_closure_policy"], "minimal_periodic_mates_only")
        self.assertEqual(record["marked_cycles_completed"], 2)
        self.assertEqual(
            [cycle["mesh_audit"]["global_cell_count"] for cycle in record["cycles"]],
            [180, 972, 4344],
        ...
        self.assertEqual(
            [cycle["coarse"]["num_nedelec_dofs"] for cycle in record["cycles"]],
            [8476, 42336, 183528],
        )
        self.assertEqual(
            [cycle["enriched"]["num_nedelec_dofs"] for cycle in record["cycles"]],
            [15405, 77980, 339850],
        )
        self.assertEqual(
            [cycle["marker"]["marked_count"] for cycle in record["cycles"]],
        ...
        self.assertEqual(
            [cycle["enriched_fixed_reference_error_l2"] for cycle in record["cycles"]],
            [
                0.022032154657776432,
                0.00715449826860017,
                0.0002203362518629887,
            ],
        )
        for cycle in record["cycles"]:
            self.assertLess(cycle["coarse"]["linear_system_relative_residual"], 1.0e-9)
            self.assertLess(cycle["enriched"]["linear_system_relative_residual"], 1.0e-9)
            self.assertEqual(cycle["mesh_audit"]["orientation"]["nonpositive_count"], 0)
        authority = record["resource_authority"]
        self.assertEqual(authority["max_observed_worker_rank_count"], 8)
        self.assertEqual(authority["max_process_tree_swap_mb"], 0.0)
        self.assertAlmostEqual(authority["memory_authority_gib"], 27.76752471923828)
        selected_cycle1 = self.p3_p4_dwr_r_mpi8["cycles"][-1]
        p5_cycle1 = record["cycles"][1]
        self.assertGreater(
            p5_cycle1["enriched_fixed_reference_error_l2"],
            selected_cycle1["enriched_fixed_reference_error_l2"],
        )
        self.assertGreater(
            p5_cycle1["enriched"]["num_nedelec_dofs"],
            selected_cycle1["enriched"]["num_nedelec_dofs"],
        )
        points = {entry["key"]: entry for entry in self.case093["points"]}
        reference = points["p4_h5"]["full3d"]["official_values"]
        p4_h7p5 = points["p4_h7p5"]["full3d"]
        ...
        final = record["cycles"][-1]
        self.assertLess(final["enriched_fixed_reference_error_l2"], structured_error)
        self.assertGreater(final["enriched"]["num_nedelec_dofs"], 2.2 * p4_h7p5["resource"]["dofs"])
        self.assertGreater(authority["memory_authority_gib"], 2.0 * p4_h7p5["resource"]["peak_memory_gib"])

    def test_p4_p5_full_cycle1_is_a_strong_hp_tradeoff(self) -> None:
        record = self.p4_p5_dwr_r_full_cycle1
        self.assertEqual(record["status"], "actual_dwr_adaptive_cycles_pass")
        self.assertTrue(record["qualification"]["pass"])
        self.assertEqual(record["qualification"]["failures"], [])
        self.assertEqual(
            record["source"]["commit_sha"],
            "96c80e9bea3ea2aa2624926d6791fc7d51268dbe",
        )
        self.assertTrue(record["source"]["stable_and_clean_after"])
        self.assertEqual(
            record["periodic_edge_closure_policy"],
        ...
        self.assertEqual(
            [cycle["enriched_fixed_reference_error_l2"] for cycle in record["cycles"]],
            [0.02203215465778368, 0.0005896036327078661],
        )
        for cycle in record["cycles"]:
            self.assertLess(cycle["coarse"]["linear_system_relative_residual"], 1.0e-9)
            self.assertLess(cycle["enriched"]["linear_system_relative_residual"], 1.0e-9)
        ...
        self.assertEqual(authority["max_observed_worker_rank_count"], 8)
        self.assertEqual(authority["max_process_tree_swap_mb"], 0.0)
        self.assertAlmostEqual(authority["memory_authority_gib"], 7.900531768798828)
        p3_p4_cycle2 = self.p3_p4_dwr_r_cycle2_tie_v1
        p3_p4_final = p3_p4_cycle2["cycles"][-1]
        final = record["cycles"][-1]
        self.assertLess(
            final["enriched"]["num_nedelec_dofs"],
            0.34 * p3_p4_final["enriched"]["num_nedelec_dofs"],
        )
        self.assertLess(
            final["enriched_fixed_reference_error_l2"],
            1.11 * p3_p4_final["enriched_fixed_reference_error_l2"],
        )
        self.assertLess(
            authority["memory_authority_gib"],
            0.43 * p3_p4_cycle2["resource_authority"]["memory_authority_gib"],
        )
        self.assertLess(record["elapsed_seconds"], 0.20 * p3_p4_cycle2["elapsed_seconds"])

    def test_p4_p5_theta07_matches_adaptive_control_but_not_structured_anchor(self) -> None:
        record = self.p4_p5_dwr_r_theta07_full_cycle1
        self.assertEqual(record["status"], "actual_dwr_adaptive_cycles_pass")
        self.assertTrue(record["qualification"]["pass"])
        self.assertEqual(record["qualification"]["failures"], [])
        self.assertEqual(
            record["source"]["commit_sha"],
            "c2898da89b055f0e6a13df3f039c6a0c24942d04",
        )
        self.assertTrue(record["source"]["stable_and_clean_after"])
        self.assertEqual(record["theta_schedule"], [0.7])
        self.assertEqual(
            record["periodic_edge_closure_policy"],
            "full_periodic_boundary_synchronization",
        )
        ...
        final = record["cycles"][-1]
        self.assertEqual(final["enriched"]["num_nedelec_dofs"], 106355)
        self.assertAlmostEqual(
            final["enriched_fixed_reference_error_l2"],
            0.0005382862236319565,
        )
        for cycle in record["cycles"]:
            self.assertLess(cycle["coarse"]["linear_system_relative_residual"], 1.0e-9)
            self.assertLess(cycle["enriched"]["linear_system_relative_residual"], 1.0e-9)
        ...
        self.assertEqual(authority["max_observed_worker_rank_count"], 8)
        self.assertEqual(authority["max_process_tree_swap_mb"], 0.0)
        self.assertAlmostEqual(authority["memory_authority_gib"], 8.08038330078125)
        theta05 = self.p4_p5_dwr_r_full_cycle1
        theta05_final = theta05["cycles"][-1]
        self.assertLess(
            final["enriched_fixed_reference_error_l2"],
            theta05_final["enriched_fixed_reference_error_l2"],
        )
        self.assertGreater(
            final["enriched"]["num_nedelec_dofs"],
            theta05_final["enriched"]["num_nedelec_dofs"],
        )
        p3_p4_cycle2 = self.p3_p4_dwr_r_cycle2_tie_v1
        p3_p4_final = p3_p4_cycle2["cycles"][-1]
        self.assertLess(
            final["enriched_fixed_reference_error_l2"],
            1.01 * p3_p4_final["enriched_fixed_reference_error_l2"],
        )
        self.assertLess(
            final["enriched"]["num_nedelec_dofs"],
            0.34 * p3_p4_final["enriched"]["num_nedelec_dofs"],
        ...
        self.assertLess(final["enriched"]["num_nedelec_dofs"], p4_h7p5["resource"]["dofs"])
        self.assertLess(authority["memory_authority_gib"], p4_h7p5["resource"]["peak_memory_gib"])

    def test_p4_p5_dwr_beats_same_degree_uniform_tetra_control(self) -> None:
        uniform = self.p4_p5_uniform_level1
        self.assertEqual(uniform["status"], "actual_uniform_tetra_control_pass")
        self.assertTrue(uniform["qualification"]["pass"])
        self.assertEqual(uniform["qualification"]["failures"], [])
        self.assertEqual(
            uniform["source"]["commit_sha"],
            "fc54cc698422d9d13477167cab4bc8566c9004b3",
        )
        self.assertTrue(uniform["source"]["stable_and_clean_after"])
        self.assertEqual(uniform["final_mesh_audit"]["global_cell_count"], 1440)
        self.assertEqual(uniform["coarse"]["num_nedelec_dofs"], 63104)
        self.assertEqual(uniform["enriched"]["num_nedelec_dofs"], 116120)
        self.assertAlmostEqual(
            uniform["coarse_fixed_reference_error_l2"],
            0.005977113445581951,
        )
        self.assertAlmostEqual(
            uniform["enriched_fixed_reference_error_l2"],
            0.0007351906785512254,
        )
        self.assertLess(uniform["coarse"]["linear_system_relative_residual"], 1.0e-9)
        self.assertLess(uniform["enriched"]["linear_system_relative_residual"], 1.0e-9)
        authority = uniform["resource_authority"]
        self.assertEqual(authority["max_observed_worker_rank_count"], 8)
        self.assertEqual(authority["max_process_tree_swap_mb"], 0.0)
        self.assertAlmostEqual(authority["memory_authority_gib"], 8.01089859008789)
        adaptive = self.p4_p5_dwr_r_theta07_full_cycle1
        adaptive_final = adaptive["cycles"][-1]
        self.assertLess(
            adaptive_final["mesh_audit"]["global_cell_count"],
            uniform["final_mesh_audit"]["global_cell_count"],
        ...
        self.assertLess(
            adaptive_final["enriched"]["num_nedelec_dofs"],
            0.92 * uniform["enriched"]["num_nedelec_dofs"],
        )
        self.assertLess(
            adaptive_final["enriched_fixed_reference_error_l2"],
            0.75 * uniform["enriched_fixed_reference_error_l2"],
        )
        self.assertLess(
            adaptive["resource_authority"]["memory_authority_gib"],
            1.02 * authority["memory_authority_gib"],
        )
