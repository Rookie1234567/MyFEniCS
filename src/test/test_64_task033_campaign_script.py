...
class Task033FormalCampaignScriptTests(unittest.TestCase):
    ...
    def test_serial_13g_no_swap_and_explicit_native_exit_contract(self) -> None:
        ...
        self.assertIn("$ExpectedRuntimeGuardBytes = 12884901888", text)
        self.assertIn("$RuntimeGuardBudgetGiB = 12.0", text)
        self.assertIn('$WarningGiB = "9.857142857142856"', text)
        self.assertIn('$TerminateGiB = "11.142857142857142"', text)
        self.assertIn("host_available_memory_bytes", text)
        self.assertIn("host_available >= 12884901888", text)
        self.assertIn("runtime_guard_budget_gib", text)
    ...
    def test_case090_and_qep_campaign_matrix(self) -> None:
        ...
        self.assertIn("Invoke-DockerHybridFunnelShardStep", text)
        self.assertIn("Assert-HybridFunnelShardOutcome", text)
        self.assertIn("Get-HybridFunnelSolverRecord", text)
        self.assertIn("-AllowHybridIntermediatePhysicalNegative:(", text)
        self.assertIn("$modeCount -in @(80, 120)", text)
        self.assertIn('$record.status -ne "physical_integration_failed"', text)
        self.assertIn("$summary.return_code -ne 2", text)
        self.assertIn("$invalidGateTypes.Count -ne 0", text)
        self.assertIn("$unexpectedFalseGates.Count -ne 0", text)
    ...
    def test_hybrid_uniform_anchors_graded_buffers_and_conditional_m240(self) -> None:
        ...
        self.assertIn("checks.candidate_pool_is_twice_requested_modes", text)
        self.assertIn("-CandidateModes (2 * $modeCount)", text)
        self.assertIn("-RequestedModes 160", text)
        self.assertIn("-CandidateModes 320", text)
