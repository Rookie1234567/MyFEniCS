...
class Task035dCase097CheckerTests(unittest.TestCase):
    ...
    def test_combined_hp_launch_contract_is_hash_bound(self) -> None:
        ...
        authority_path = ROOT / TASK035D_COMBINED_HP_AUTHORITY_PATH
        plan = json.loads(plan_path.read_text(encoding="utf-8"))
        authority = json.loads(
            authority_path.read_text(encoding="utf-8")
        )
        embedded = task035d_case097_combined_hp_plan_authority_gate(
            plan,
            authority,
            expected_plan_file_sha256=(
                TASK035D_COMBINED_HP_PLAN_FILE_SHA256
            ...
            authority_path_from_root=TASK035D_COMBINED_HP_AUTHORITY_PATH,
        )
        self.assertTrue(embedded["pass"], embedded["failures"])
        command = [
            "mpiexec",
        ...
        contract = _candidate_launch_contract(
            {
                "command": command,
                "task035d_candidate_id": TASK035D_COMBINED_HP_PLAN_NAME,
                "task035d_case097_launch_gate": embedded,
                "resource_policy": {"swap_allowed": False},
                "no_swap": True,
                "task035d_accuracy_credit": (
    ...
    def test_final_evaluator_requires_all_12_channels(self) -> None:
        ...
        combined = evaluate_task035d_case097_candidate(
            watchdog=watchdog,
            launch_gate=_pass_payload(),
            solver_gate={
                "pass": True,
