...
def _audit():
    return {
        ...
        "reported_history": [[0, 1.0], [10, 0.5], [20, 0.1]],
        "condensed_true_samples": [[0, 1.0], [10, 0.5], [20, 0.1]],
        "final": {
            "converged_reason": -3,
            "iterations": 20,
...
def test_parser_scope_and_worker_command(tmp_path):
    ...
    args = watchdog._parse_args(valid)
    ...
    assert command[position + 1] == "20"
    bad_iterations = valid.copy()
    bad_iterations[bad_iterations.index("--task037-f3-screen") + 1] = "100"
...
def test_f3_qualification_uses_core_audit_gate():
    ...
    assert not watchdog._task037_f3_screen_gate(bad)["finite_and_scale"]
