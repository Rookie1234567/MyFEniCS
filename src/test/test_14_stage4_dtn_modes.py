...
class Stage4DtnModeTests(unittest.TestCase):
    ...
    def test_sampled_p_mode_projection_uses_only_tangential_components(self):
        ...
        sample_phases = np.asarray(
            (1.0 + 0.0j, 0.0 + 1.0j), dtype=np.complex128
        )
        mode_samples = sample_phases[:, None] * p_mode.e_vector[None, :]
        expected = 2.0 - 3.0j
        electric_samples = expected * mode_samples
        electric_samples[:, 2] = np.asarray(
            (1.0e6 + 2.0e6j, -3.0e6 + 4.0e6j)
        )
        weights = np.asarray((0.25, 2.0), dtype=np.float64)

        projection = _sampled_tangential_projection(
            electric_samples, mode_samples, weights
        )
        full_vector_projection = np.sum(
            weights
            * np.sum(electric_samples * np.conj(mode_samples), axis=1)
        ) / np.sum(
            weights * np.sum(np.abs(mode_samples) ** 2, axis=1)
        )
    ...
    def test_outgoing_projection_subtracts_incident_only_on_top(self):
        ...
        self.assertEqual(
            _outgoing_projection(total, incident, "top"), 2.0 + 7.0j
        )
        self.assertEqual(
            _outgoing_projection(total, incident, "bottom"), total
        )
    ...
    def test_sampled_projection_rejects_zero_tangential_mode_norm(self):
        electric_samples = np.asarray(
            ((1.0, 2.0, 3.0),), dtype=np.complex128
        )
        normal_only_mode = np.asarray(
            ((0.0, 0.0, 4.0 + 5.0j),), dtype=np.complex128
        )

        with self.assertRaisesRegex(ValueError, "zero tangential norm"):
            _sampled_tangential_projection(
                electric_samples, normal_only_mode
            )
