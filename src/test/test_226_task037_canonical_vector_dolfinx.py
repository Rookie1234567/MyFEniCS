...
class TestTask037CanonicalVectorDolfinx(unittest.TestCase):
    ...
    def test_real_floquet_phases_and_physical_relation_keys(self) -> None:
        ...
        if face_block is None:
            ...
            canonical = (
                np.arange(physical_transform.shape[1], dtype=np.complex128)
            ...
            old = (
                np.linalg.solve(np.asarray(face_block.coefficient_transform), stored)
                / phase
            )
    ...
    def test_target_topology_multi_master_block_orientation(self) -> None:
        ...
        if candidates:
            ...
            physical_transform, _physical_state = (
                canonical_module._physical_entity_transform(coords, 2, 6, tolerance)
            )
