...
class Task035bHighOrderResourceAuditTests(unittest.TestCase):
    ...
    def test_non_exact_p4_trace_p6_interior_is_not_a_public_space(
        self,
    ) -> None:
        cfg = replace(
            self.cfg,
        ...
        with self.assertRaisesRegex(
            ...
            _create_nedelec_space(self.mesh_data.mesh, cfg)
    ...
    def test_formal_mpi8_p6_condensation_record_is_full_system_qualified(
        ...
        self.assertEqual(p6["matrix_stats"]["matrix_rows"], 51272)
        self.assertEqual(p6["matrix_stats"]["matrix_nnz_used"], 41989040.0)
        ...
        self.assertEqual(audit["full_rows"], 173802)
        self.assertEqual(audit["interior_rows"], 113400)
        self.assertEqual(audit["trace_rows"], 60402)
