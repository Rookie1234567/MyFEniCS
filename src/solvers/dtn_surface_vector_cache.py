"""Fail-closed persistent cache for reduced DtN modal bundles.

The Stage-4 auxiliary DtN operator repeatedly assembles the same two
unconstrained tangential component vectors, applies the same left/right
cell-Schur and Floquet reductions, and evaluates the same 2-by-2
cell-interior bilinear for every polarization of a Floquet order.  In the
Task035b assembly-time-condensed path all of those results can be reused
across identical offline/online runs.

This cache is deliberately opt-in and rank-partition-bound.  It stores only
owned entries of the original unconstrained FE vectors together with the
owned sparse reduced entries and the globally closed cell-interior
bilinear.  It publishes a manifest after the payload, verifies a logical
content checksum, and treats every identity or payload discrepancy as a
cache miss.  The ordinary DtN path does not instantiate this class.
"""
...
_CACHE_SCHEMA = "task035b.dtn-reduced-modal-persistent-cache.v2"
_MANIFEST_SCHEMA = "task035b.dtn-reduced-modal-cache-manifest.v2"
...
def _bundle_content_sha256(
    ...
    digest = hashlib.sha256()
    digest.update(b"task035b.dtn-reduced-modal-content.v2\0")
    for name in sorted(arrays):
        ...
        digest.update(
            _array_content_sha256(
                canonical,
                namespace=b"task035b.dtn-reduced-modal-array.v2",
            ).encode("ascii")
        )
        digest.update(b"\0")
    return digest.hexdigest()
...
def dtn_reduced_operator_identity(
    condensed: Any,
) -> dict[str, Any]:
    """Hash every rank-local numerical input to modal Schur reduction."""

    digest = hashlib.sha256()
    digest.update(b"task035b.dtn-reduced-operator-identity.v1\0")
    dimensions = {
        "full_rows": int(condensed.full_rows),
        "trace_rows": int(condensed.trace_rows),
        "active_rows": int(condensed.active_rows),
        "appended_rows": int(condensed.appended_rows),
        "interior_rows": int(condensed.interior_rows),
        "active_interior_rows": int(condensed.active_interior_rows),
    }
    digest.update(
        _canonical_json(_canonical_value(dimensions)).encode("ascii")
    )
    digest.update(b"\0")

    cell_digest = hashlib.sha256()
    cell_digest.update(b"task035b.dtn-reduced-operator-cells.v1\0")
    for cell in condensed.cell_recovery_maps:
        ...
        for name, values in (
            ...
            cell_digest.update(name.encode("ascii"))
            cell_digest.update(b"\0")
            cell_digest.update(
                _array_content_sha256(
                    np.asarray(values),
                    namespace=(
                        b"task035b.dtn-reduced-operator-cell-array.v1"
                    ),
                ).encode("ascii")
            )
            cell_digest.update(b"\0")
    cell_content_sha256 = cell_digest.hexdigest()
    digest.update(cell_content_sha256.encode("ascii"))
    digest.update(b"\0")

    class_arrays: tuple[
        tuple[str, Mapping[Any, Any], bool],
        ...,
    ] = (
        (
            "interior_from_trace",
    ...
    class_array_count = 0
    for namespace, mapping, values_are_tuple in class_arrays:
        digest.update(namespace.encode("ascii"))
        digest.update(b"\0")
        ordered = sorted(
            mapping.items(),
            key=lambda item: _canonical_json(
                _canonical_value(item[0])
            ),
        )
        for class_key, stored in ordered:
            digest.update(
                _canonical_json(
                    _canonical_value(class_key)
                ).encode("ascii")
            )
            digest.update(b"\0")
            arrays = stored if values_are_tuple else (stored,)
            for array_index, values in enumerate(arrays):
                digest.update(
                    _array_content_sha256(
                        np.asarray(values),
                        namespace=(
                            b"task035b.dtn-reduced-operator-class-array.v1."
                            + namespace.encode("ascii")
                            + b"."
                            + str(array_index).encode("ascii")
                        ),
                    ).encode("ascii")
                )
                digest.update(b"\0")
    ...
    return {
        ...
        "cell_recovery_map_content_sha256": cell_content_sha256,
        "cell_recovery_map_count": len(
            condensed.cell_recovery_maps
        ),
        "class_array_count": class_array_count,
...
class PersistentDtnSurfaceVectorCache:
    """One rank-local, content-addressed reduced DtN modal bundle."""
    ...
    def __init__(
        ...
        *,
        ...
        reduced_operator_identity: Mapping[str, Any],
        ...
        canonical_operator_identity = _canonical_value(
            dict(reduced_operator_identity)
        )
        if (
            canonical_operator_identity.get("schema_version")
            != "task035b.dtn-reduced-operator-identity.v1"
            or _SHA256_PATTERN.fullmatch(
                str(
                    canonical_operator_identity.get(
                        "content_sha256",
                        "",
                    )
                )
            ...
            is None
            or canonical_operator_identity.get(
                "material_and_tensor_content_bound"
            )
            is not True
        ):
            raise ValueError(
                "DtN reduced-modal cache requires a qualified material/"
                "tensor-bound reduced-operator identity"
            )
        ...
        self._vector_array_names = tuple(
            f"vector_{index:04d}"
            for index in range(len(self.descriptors))
        )
        order_components: dict[str, dict[int, int]] = {}
        order_keys: list[str] = []
        for index, descriptor in enumerate(self.descriptors):
            component = descriptor.get("component")
            if component not in {0, 1}:
                raise ValueError(
                    "DtN reduced-modal cache descriptors require "
                    "components 0 and 1"
                )
            order_descriptor = dict(descriptor)
            del order_descriptor["component"]
            order_key = _canonical_json(order_descriptor)
            if order_key not in order_components:
                order_components[order_key] = {}
                order_keys.append(order_key)
            if int(component) in order_components[order_key]:
                raise ValueError(
                    "DtN reduced-modal cache has a duplicate component "
                    "within one surface order"
                )
            order_components[order_key][int(component)] = index
        if any(
            set(components) != {0, 1}
            for components in order_components.values()
        ):
            raise ValueError(
                "DtN reduced-modal cache requires a complete x/y "
                "descriptor pair for every surface order"
            )
        self._order_descriptor_indices = tuple(
            (
                order_components[key][0],
                order_components[key][1],
            )
            for key in order_keys
        )
        self._descriptor_pair_to_order = {
            pair: order
            for order, pair in enumerate(
                self._order_descriptor_indices
            )
        }
        reduced_entry_names: list[str] = []
        for index in range(len(self.descriptors)):
            for side in ("right", "left"):
                reduced_entry_names.extend(
                    (
                        f"{side}_rows_{index:04d}",
                        f"{side}_values_{index:04d}",
                    )
                )
        self._reduced_entry_array_names = tuple(reduced_entry_names)
        self._bilinear_array_names = tuple(
            f"interior_bilinear_{order:04d}"
            for order in range(len(self._order_descriptor_indices))
        )
        self._array_names = (
            self._vector_array_names
            + self._reduced_entry_array_names
            + self._bilinear_array_names
        )
        ...
        self._active_rows = int(trace_constraints.active_rows)
        owned_active_rows = getattr(
            trace_constraints,
            "owned_active_rows",
            None,
        ...
        local_active_rows = len(owned_active_rows)
        active_counts = tuple(
            int(value)
            for value in self.comm.allgather(local_active_rows)
        )
        self._active_ownership_range = [
            int(sum(active_counts[: self.comm.rank])),
            int(sum(active_counts[: self.comm.rank + 1])),
        ]
        if sum(active_counts) != self._active_rows:
            raise ValueError(
                "DtN reduced-modal cache active-row partition does not "
                "close globally"
            )

        self.identity = {
            "schema_version": (
                "task035b.dtn-reduced-modal-cache-identity.v2"
            ),
            ...
            "reduced_operator": canonical_operator_identity,
            "vector_layout": {
                "global_size": self._global_size,
                "local_owned_size": self._owned_size,
                "ownership_range": self._ownership_range,
            },
            "reduced_modal_layout": {
                "global_active_rows": self._active_rows,
                "local_owned_active_rows": local_active_rows,
                "active_ownership_range": (
                    self._active_ownership_range
                ),
                "surface_order_count": len(
                    self._order_descriptor_indices
                ),
                "component_count": len(self.descriptors),
                "stored_quantities": [
                    "unconstrained_owned_component_vectors",
                    "right_condensed_owned_sparse_entries",
                    "left_condensed_owned_sparse_entries",
                    "global_2x2_cell_interior_schur_bilinear",
                ],
            },
            "abi": {
        ...
        self.payload_path = self.directory / (
            f"dtn_reduced_modal_{self.identity_sha256}.npz"
        )
        ...
        self._recorded: dict[str, np.ndarray] = {}
        ...
        self._restores = 0
        self._records = 0
        self._reduced_bundle_restores = 0
        self._reduced_bundle_records = 0
        self._unrestored_vector_arrays = 0
        self._writes = 0
    ...
    def _load_bundle(
        ...
        if manifest.get("arrays") != actual_metadata:
        ...
        for name in self._vector_array_names:
            array = arrays[name]
            if (
                array.shape != (self._owned_size,)
                or array.dtype != np.dtype(np.complex128)
                or not np.all(np.isfinite(array))
            ):
                return None, "full_vector_shape_dtype_or_finite_mismatch"
        active_start, active_end = self._active_ownership_range
        for index in range(len(self.descriptors)):
            for side in ("right", "left"):
                rows = arrays[f"{side}_rows_{index:04d}"]
                values = arrays[f"{side}_values_{index:04d}"]
                if (
                    rows.ndim != 1
                    or rows.dtype != np.dtype(PETSc.IntType)
                    or values.shape != rows.shape
                    or values.dtype != np.dtype(np.complex128)
                    or not np.all(np.isfinite(values))
                ):
                    return (
                        None,
                        "reduced_entries_shape_dtype_or_finite_mismatch",
                    )
                if len(rows) and (
                    int(rows[0]) < active_start
                    or int(rows[-1]) >= active_end
                    or np.any(rows[1:] <= rows[:-1])
                ):
                    return None, "reduced_entry_row_ownership_mismatch"
        for name in self._bilinear_array_names:
            bilinear = arrays[name]
            if (
                bilinear.shape != (2, 2)
                or bilinear.dtype != np.dtype(np.complex128)
                or not np.all(np.isfinite(bilinear))
            ):
                return None, "interior_bilinear_shape_dtype_or_finite_mismatch"
        return {
    ...
    def restore_vector(
        ...
        array_name = self._vector_array_names[index]
        try:
            values = self._loaded.pop(array_name)
        except KeyError as error:
            raise RuntimeError(
    ...
    def record_vector(
        ...
        index = self._descriptor_index(descriptor)
        array_name = self._vector_array_names[index]
        if array_name in self._recorded:
            raise RuntimeError(
        ...
        self._recorded[array_name] = np.ascontiguousarray(owned).copy()
        self._records += 1

    def _descriptor_pair_order(
        self,
        descriptors: Sequence[Mapping[str, Any]],
    ) -> tuple[int, tuple[int, int]]:
        if len(descriptors) != 2:
            raise ValueError(
                "DtN reduced-modal bundle requires exactly two "
                "component descriptors"
            )
        indices = tuple(
            self._descriptor_index(descriptor)
            for descriptor in descriptors
        )
        try:
            order = self._descriptor_pair_to_order[indices]
        except KeyError as error:
            raise ValueError(
                "DtN reduced-modal descriptors are not the ordered x/y "
                "pair from one surface order"
            ) from error
        return order, indices

    def _validated_reduced_entries(
        self,
        entries: tuple[np.ndarray, np.ndarray],
    ) -> tuple[np.ndarray, np.ndarray]:
        rows = np.ascontiguousarray(
            np.asarray(entries[0], dtype=PETSc.IntType)
        )
        values = np.ascontiguousarray(
            np.asarray(entries[1], dtype=np.complex128)
        )
        if rows.ndim != 1 or values.shape != rows.shape:
            raise ValueError(
                "DtN reduced-modal sparse rows and values must be "
                "same-length vectors"
            )
        if not np.all(np.isfinite(values)):
            raise ValueError("DtN reduced-modal sparse values must be finite")
        active_start, active_end = self._active_ownership_range
        if len(rows) and (
            int(rows[0]) < active_start
            or int(rows[-1]) >= active_end
            ...
            raise ValueError(
                "DtN reduced-modal sparse rows must be strictly sorted "
                "and locally owned active rows"
            )
        return rows.copy(), values.copy()

    def record_reduced_modal_bundle(
        self,
        descriptors: Sequence[Mapping[str, Any]],
        *,
        right_entries: Sequence[tuple[np.ndarray, np.ndarray]],
        left_entries: Sequence[tuple[np.ndarray, np.ndarray]],
        interior_bilinear: np.ndarray,
    ) -> None:
        """Record one fully reduced x/y component bundle on a cache miss."""

        names_and_entries: list[
            tuple[str, tuple[np.ndarray, np.ndarray]]
        ] = []
        bilinear_name = ""
        bilinear = np.empty((0, 0), dtype=np.complex128)
        local_error: dict[str, str] | None = None
        try:
            if self._hit:
                raise RuntimeError(
                    "DtN reduced-modal cache may not record after a hit"
                )
            order, indices = self._descriptor_pair_order(descriptors)
            if len(right_entries) != 2 or len(left_entries) != 2:
                raise ValueError(
                    "DtN reduced-modal bundle requires two right and two "
                    "left sparse entry sets"
                )
            for side, entry_sets in (
                ("right", right_entries),
                ("left", left_entries),
            ):
                for index, entries in zip(
                    indices,
                    entry_sets,
                    strict=True,
                ):
                    rows_name = f"{side}_rows_{index:04d}"
                    values_name = f"{side}_values_{index:04d}"
                    names_and_entries.append(
                        (
                            rows_name,
                            self._validated_reduced_entries(entries),
                        )
                    )
                    if (
                        rows_name in self._recorded
                        or values_name in self._recorded
                    ):
                        raise RuntimeError(
                            "DtN reduced-modal bundle was recorded twice"
                        )
            bilinear_name = self._bilinear_array_names[order]
            if bilinear_name in self._recorded:
                raise RuntimeError(
                    "DtN reduced-modal bilinear was recorded twice"
                )
            bilinear = np.ascontiguousarray(
                np.asarray(interior_bilinear, dtype=np.complex128)
            )
            if bilinear.shape != (2, 2) or not np.all(
                np.isfinite(bilinear)
            ):
                raise ValueError(
                    "DtN reduced-modal interior bilinear must be a finite "
                    "2-by-2 complex matrix"
                )
        except Exception as error:
            local_error = {
        ...
        if any(error is not None for error in errors):
            raise RuntimeError(
                ...
                + "; ".join(
                    f"rank {rank}: {error['type']}: {error['message']}"
                    for rank, error in enumerate(errors)
                    if error is not None
                )
            )
        for rows_name, (rows, values) in names_and_entries:
            values_name = rows_name.replace("_rows_", "_values_")
            self._recorded[rows_name] = rows
            self._recorded[values_name] = values
        self._recorded[bilinear_name] = bilinear.copy()
        self._reduced_bundle_records += 1

    def restore_reduced_modal_bundle(
        self,
        descriptors: Sequence[Mapping[str, Any]],
    ) -> tuple[
        tuple[
        ...
        """Restore sparse left/right projections and the 2-by-2 bilinear."""

        if not self._hit or self._loaded is None:
            raise RuntimeError(
                "DtN reduced-modal restore requires a validated hit"
            )
        order, indices = self._descriptor_pair_order(descriptors)
        restored: dict[
            str,
        ...
        try:
            ...
            bilinear = self._loaded.pop(
                self._bilinear_array_names[order]
            )
        except KeyError as error:
            raise RuntimeError(
                "DtN reduced-modal bundle was restored twice or is "
                "incomplete"
            ) from error
        self._reduced_bundle_restores += 1
        return restored["right"], restored["left"], bilinear

    def _write_bundle(self) -> None:
        missing = sorted(set(self._array_names) - set(self._recorded))
        extra = sorted(set(self._recorded) - set(self._array_names))
        if missing or extra:
            raise RuntimeError(
                "DtN reduced-modal cache cannot publish an incomplete "
                "or unexpected bundle; "
                f"missing arrays {missing[:8]}, extra arrays {extra[:8]}"
            )
        arrays = {
            name: self._recorded[name]
            for name in self._array_names
        }
        suffix = (
    ...
    def finalize(self) -> dict[str, Any]:
        ...
        remaining_loaded_names = (
            set() if self._loaded is None else set(self._loaded)
        )
        remaining_reduced = sorted(
            remaining_loaded_names - set(self._vector_array_names)
        )
        if self._hit and remaining_reduced:
            raise RuntimeError(
                "validated DtN cache hit did not consume every reduced "
                f"modal array; {remaining_reduced[:8]} remain"
            )
        self._unrestored_vector_arrays = len(
            remaining_loaded_names & set(self._vector_array_names)
        )
        self._loaded = None
        ...
        reduced_bundle_restore_count = int(self.comm.allreduce(
            self._reduced_bundle_restores,
            op=MPI.SUM,
        ))
        reduced_bundle_record_count = int(self.comm.allreduce(
            self._reduced_bundle_records,
            op=MPI.SUM,
        ))
        unrestored_vector_array_count = int(self.comm.allreduce(
            self._unrestored_vector_arrays,
            op=MPI.SUM,
        ))
        ...
        return {
            ...
            "reduced_operator_content_sha256_by_rank": (
                self.comm.allgather(
                    self.identity["reduced_operator"][
                        "content_sha256"
                    ]
                )
            ),
            "identity_binds": [
                ...
                "material_tensor_aii_and_primal_dual_reduction_content",
                "mpi_rank_partition_and_full_vector_ownership",
                "mpi_rank_partition_and_active_trace_ownership",
            ...
            "reduced_bundle_restore_count_sum": (
                reduced_bundle_restore_count
            ),
            "reduced_bundle_record_count_sum": (
                reduced_bundle_record_count
            ),
            "unrestored_full_vector_array_count_sum": (
                unrestored_vector_array_count
            ),
            "write_count_sum": write_count,
            "descriptor_count_per_rank": len(self.descriptors),
            "surface_order_count_per_rank": len(
                self._order_descriptor_indices
            ),
            "full_vector_array_count_per_rank": len(
                self._vector_array_names
            ),
            "reduced_array_count_per_rank": (
                len(self._reduced_entry_array_names)
                + len(self._bilinear_array_names)
            ),
            ...
            "stages_skipped_on_hit": [
                ...
                "left_and_right_cell_schur_trace_projection",
                "cell_interior_2x2_schur_bilinear",
            ],
            "trace_projection_recomputed_after_restore": False,
            "cell_interior_bilinear_recomputed_after_restore": False,
            "full_vector_restore_policy": (
                "restore_only_for_nonzero_incident_rhs_or_explicit_"
                "dual_recovery_context"
            ),
...
def disabled_dtn_surface_vector_cache_audit() -> dict[str, Any]:
    ...
    return {
        ...
        "material_and_tensor_identity_bound": False,
        "restore_count_sum": 0,
        "record_count_sum": 0,
        "reduced_bundle_restore_count_sum": 0,
        "reduced_bundle_record_count_sum": 0,
        "unrestored_full_vector_array_count_sum": 0,
        "write_count_sum": 0,
