...
from ..constraints.floquet_constraint import build_floquet_constraints, dof_trace_mismatch
...
from .solve_vector_maxwell import _json_default, _petsc_to_csr, _solve_manual, _solve_mpc
...
def _compressed_outer_trace_triplets(trace: CompressedTraceVector, coefficient: complex):
...
def _add_compressed_trace_to_rhs(b_out: np.ndarray, trace: CompressedTraceVector, coefficient: complex) -> None:
    nz = np.asarray(trace["indices"], dtype=np.int64)
    values = np.asarray(trace["values"], dtype=np.complex128)
    if len(nz):
...
def _port_side_specs(cfg: SimulationConfig) -> tuple[tuple[str, int, complex], tuple[str, int, complex]]:
    return (
...
def _is_near_rayleigh(k_medium: complex, alpha: complex, cfg: SimulationConfig) -> bool:
    ...
    return abs(beta_squared) <= float(cfg.port_rayleigh_tolerance) * _rayleigh_scale(k_medium)


def _is_clearly_propagating(k_medium: complex, alpha: complex, beta: complex, cfg: SimulationConfig) -> bool:
    if abs(complex(alpha).imag) > 1e-10:
        return False
    if abs(complex(k_medium).imag) > 1e-10:
        return False
    margin = float(cfg.port_rayleigh_tolerance) * max(abs(k_medium), 1.0)
    return abs(float(np.real(alpha))) < abs(float(np.real(k_medium))) - margin and beta.real > margin
...
def _select_dtn_port_modes(cfg: SimulationConfig, log) -> dict[str, object]:
    ...
    for side, tag, refractive_index in _port_side_specs(cfg):
        ...
        for order in side_candidates:
            ...
            selected = order == 0 or (cfg.port_use_diffraction_orders and bool(probe["is_propagating"]))
...
def _top_incident_source_amplitude(mode: dict[str, object], cfg: SimulationConfig) -> complex:
...
def _build_dtn_trace_data(V, mesh_data, cfg: SimulationConfig, log) -> dict[str, object]:
    if cfg.use_pml:
        raise RuntimeError(
            "Fourier DtN ports require use_pml=False in the current port total-field solver."
        )
    if MPI.COMM_WORLD.size != 1:
        raise RuntimeError("Fourier DtN ports are currently implemented in the serial manual backend.")
    ...
    for raw_mode in selection["modes"]:
        ...
        ell = _fourier_trace_vector(V, mesh_data, int(mode["tag"]), complex(mode["alpha"]))
...
def _add_fourier_port_operators_explicit(A_csr, b_np, V, mesh_data, cfg: SimulationConfig, log):
    ...
    for mode in metadata["modes"]:
        ...
        if side == "top" and order == 0:
            _add_compressed_trace_to_rhs(b_out, trace, -_top_incident_source_amplitude(mode, cfg))
...
def _add_fourier_port_operators_auxiliary(A_csr, b_np, V, mesh_data, cfg: SimulationConfig, log):
    metadata = _build_dtn_trace_data(V, mesh_data, cfg, log)
    b_out = np.asarray(b_np, dtype=np.complex128).copy()
    n_fem = A_csr.shape[0]
    ...
    A_aug = sparse.block_diag((A_csr, sparse.csr_matrix((n_aux, n_aux), dtype=np.complex128)), format="csr")
    A_aug = A_aug + A_aux
...
def _fem_constraint_embedding(n_fem: int, constraints):
    ...
    C_fem = sparse.coo_matrix((data, (rows, cols)), shape=(n_fem, len(free)), dtype=np.complex128).tocsr()
    return C_fem, len(free)


def _solve_manual_with_auxiliary(A_aug, b_aug, constraints, n_fem: int):
    if MPI.COMM_WORLD.size != 1:
        raise RuntimeError("auxiliary DtN manual constraint elimination is serial-only.")
    n_total = A_aug.shape[0]
    n_aux = n_total - n_fem
    C_fem, num_fem_reduced = _fem_constraint_embedding(n_fem, constraints)
    C_aug = sparse.block_diag((C_fem, sparse.identity(n_aux, dtype=np.complex128, format="csr")), format="csr")
    ...
    return x_full[:n_fem], x_full[n_fem:], {
...
def _auxiliary_coefficients_by_side(modes: list[dict[str, object]], aux_values: np.ndarray) -> dict[str, dict[int, complex]]:
...
def _write_auxiliary_amplitudes(out_dir: Path, modes: list[dict[str, object]], aux_values: np.ndarray) -> None:
...
def _add_fourier_port_operators(A_csr, b_np, V, mesh_data, cfg: SimulationConfig, log):
    if cfg.use_pml:
        raise RuntimeError("多衍射级次 Fourier 端口目前要求 use_pml=False；请去掉 --port-use-pml。")
    if MPI.COMM_WORLD.size != 1:
...
def run_port_case(
    ...
    if cfg.polarization_type.upper() != "TM":
        raise RuntimeError("solve_port_maxwell.run_port_case() only supports TM Ex/Ey; use solve_te_maxwell for TE.")
    if cfg.use_pml:
        raise RuntimeError(
    ...
    log(f"formulation = port_total_field")
    ...
    if cfg.port_boundary_model not in ("robin", "dtn"):
        raise ValueError("A concrete port case must use port_boundary_model='robin' or 'dtn'.")
    if cfg.port_dtn_order_count < 0:
    ...
    if cfg.port_boundary_model == "dtn" and constraint_backend in ("mpc_official", "mpc_lowlevel"):
        raise RuntimeError("DtN Fourier 端口目前只支持 manual 后端；官方 MPC 后端请使用 port_boundary_model='robin'。")
    ...
    if constraint_backend in ("mpc_official", "mpc_lowlevel"):
        ...
        port_auxiliary_coefficients: dict[str, dict[int, complex]] = {"top": {}, "bottom": {}}
        ...
        if cfg.port_boundary_model == "dtn":
            if cfg.port_dtn_assembly == "explicit":
                ...
                log("solving explicit DtN constrained port system with C^H A C reduction + SciPy SuperLU")
                ...
                port_modes = port_metadata["modes"]
                port_trace_vectors = port_metadata["trace_vectors"]
                log("solving auxiliary DtN constrained port system with block C^H A C reduction + SciPy SuperLU")
                solution, port_auxiliary_values, solver_info = _solve_manual_with_auxiliary(
                    A_aug, b_aug, constraints, A_csr.shape[0]
                )
                port_auxiliary_coefficients = _auxiliary_coefficients_by_side(port_modes, port_auxiliary_values)
                _write_auxiliary_amplitudes(out_dir, port_modes, port_auxiliary_values)
        else:
            log("solving constrained port system with C^H A C reduction + SciPy SuperLU")
            solution, solver_info = _solve_manual(A_csr, b_np, constraints)
        ...
        raise ValueError("port total-field solver supports 'mpc_official', 'mpc_lowlevel', or 'manual'.")
    if constraint_backend in ("mpc_official", "mpc_lowlevel"):
        port_modes = []
    ...
    field_metrics = save_fields_and_plots(mesh_data, cfg, E_inc_output, E_scat_output, E_total, out_dir)
    ...
    if cfg.port_boundary_model == "dtn":
        ...
        if cfg.port_dtn_assembly == "auxiliary":
            dtn_auxiliary_power_metrics = compute_dtn_auxiliary_power_metrics(
                mesh_data, cfg, E_total, out_dir, port_auxiliary_coefficients, port_metadata
            )
            if (
                {"R_total", "T_total", "R_plus_T"}.issubset(dtn_port_power_metrics)
                and {"R_total", "T_total", "R_plus_T"}.issubset(dtn_auxiliary_power_metrics)
            ):
                dtn_auxiliary_vs_trace_power_difference = {
                    "R_total_aux_minus_trace": (
                        dtn_auxiliary_power_metrics["R_total"] - dtn_port_power_metrics["R_total"]
                    ),
                    "T_total_aux_minus_trace": (
                        dtn_auxiliary_power_metrics["T_total"] - dtn_port_power_metrics["T_total"]
                    ),
                    "R_plus_T_aux_minus_trace": (
                        dtn_auxiliary_power_metrics["R_plus_T"] - dtn_port_power_metrics["R_plus_T"]
                    ),
                }
        if (
    ...
    summary = {
        ...
        "num_auxiliary_dofs": solver_info.get("num_auxiliary_dofs", port_metadata.get("num_auxiliary_dofs", 0)),
