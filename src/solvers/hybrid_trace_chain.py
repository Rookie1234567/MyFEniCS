"""Research-only matrix-free full-FE trace-chain correctness oracle."""
...
def solve_block_tridiagonal_recursive(
    ...
    """Solve a research-oracle block-tridiagonal system by Schur recursion."""

    diagonal = tuple(
        np.asarray(block, dtype=np.complex128) for block in diagonal_blocks
    )
    ...
    solution: list[np.ndarray] = [
        np.empty_like(reduced_rhs[0]) for _ in range(block_count)
    ]
    solution[-1] = lu_solve(
        factors[-1],
        reduced_rhs[-1],
...
def solve_block_tridiagonal_recursive_mpi(
    ...
    *,
    ...
    """Solve the oracle chain with MPI-sharded Schur-complement columns."""
...
class PairedEndpointSchurAction:
    """Research-only concurrent endpoint action on paired MPI subgroups."""
    ...
    @staticmethod
    def _local_apply(
        action: Any, transfer: Any, columns: np.ndarray
    ) -> np.ndarray | None:
        if action is None:
            return None
    ...
    def apply_pair(
        ...
        top_local = self._local_apply(self.top_action, self.top_transfer, top_columns)
...
class FullFeTraceChainAction:
    """Research-only repeated-cell full-FE trace-chain matrix-free action."""
    ...
    def __init__(
        ...
        *,
        ...
        if self.cell_count <= 0:
            raise ValueError("At least one repeated cell is required.")
        self.plane_size = int(getattr(cell_action, "left_rows"))
        if (
            self.plane_size <= 0
            or int(getattr(cell_action, "right_rows")) != self.plane_size
        ):
        ...
        if int(getattr(cell_transfer, "source_size")) != self.plane_size:
            raise ValueError(
                "The cell transfer source size does not match the FE plane."
            )
        ...
        endpoint_actions = (
            ...
            if paired_endpoints is None
                ...
                paired_endpoints.bottom_action,
                paired_endpoints.top_action,
            )
        )
        for endpoint_action in endpoint_actions:
            ...
            if canonical_sign is not None and canonical_sign != 1:
                raise ValueError(
                    "Endpoint actions must use raw-outward canonical_sign=+1."
                )
        ...
        endpoint_transfers = (
            (
                (bottom_transfer, bottom_action),
                (top_transfer, top_action),
            )
            if paired_endpoints is None
            else (
                (
                    paired_endpoints.bottom_transfer,
                    paired_endpoints.bottom_action,
                ),
                (
                    paired_endpoints.top_transfer,
                    paired_endpoints.top_action,
                ),
            )
        )
        for transfer, action in endpoint_transfers:
            if transfer is None or action is None:
                if paired_endpoints is not None:
                    continue
                raise ValueError("Both endpoint actions and transfers are required.")
            if int(getattr(transfer, "source_size")) != self.plane_size:
                raise ValueError(
                    "An endpoint transfer source does not match the FE plane."
                )
            if int(getattr(transfer, "target_size")) != action_rows(action):
                raise ValueError(
                    "An endpoint transfer target does not match its action."
                )
    ...
    def apply_columns(self, values: np.ndarray) -> np.ndarray:
        """Apply the oracle chain to replicated FE-plane columns."""
        ...
        for cell in range(self.cell_count):
            ...
            result[cell] += port_output[: self.plane_size]
            result[cell + 1] += self.cell_transfer.dual(
                port_output[self.plane_size :, :]
            )
    ...
    def mult(self, _mat: Any, x: Any, y: Any) -> None:
        """PETSc MatPython adapter for one replicated oracle column."""
    ...
    def build_compact_trace_blocks(
        ...
        *,
        ...
        """Materialize only the five unique oracle/debug trace blocks."""
        ...
        if self.paired_endpoints is not None:
            ...
            bottom_diagonal, top_diagonal = self.paired_endpoints.apply_diagonal_pair(
                basis
            )
            endpoint_diagonals = {
                0: bottom_diagonal,
    ...
    def destroy(self, _obj: Any = None) -> None:
        """Release owned actions once; transfer objects remain non-owning."""
