"""Sparse endpoint trace mass and Riesz actions for Hybrid ports."""
...
class EndpointTraceMassSelection:
    """Rows and facet tag defining one endpoint trace mass."""
...
class EndpointTraceMassAction:
    """Sparse endpoint mass with one reusable CHOLMOD Riesz solver."""
    ...
    def solve_columns(self, values: np.ndarray) -> np.ndarray:
        """Apply the sparse Riesz inverse to a small set of columns."""
        ...
        try:
            ...
            for column in range(columns.shape[1]):
                ...
                result[:, column] = np.asarray(
                    solution.getValues(
                        np.arange(self.shape[0], dtype=PETSc.IntType)
                    ),
                    dtype=np.complex128,
                )
        finally:
    ...
    def multiply_columns(self, values: np.ndarray) -> np.ndarray:
        """Apply the sparse endpoint mass to a small set of columns."""
...
def _build_one_action(
    ...
    prefix = f"task036_endpoint_mass_{probe_offset}_"
    ...
    try:
        if any(
            ...
            raise AssertionError("Endpoint mass qualification requires sparse AIJ matrices.")
        if face_mass.getSize() != (1250, 1250):
            raise AssertionError("Endpoint full mass shape is not 1250x1250.")
        if transform.getSize() != (1250, 1200):
            raise AssertionError("Endpoint constraint shape is not 1250x1200.")
        if reduced.getSize() != (1200, 1200):
            raise AssertionError("Endpoint reduced mass shape is not 1200x1200.")
        hermitian_defect = float(
            difference.norm() / max(reduced.norm(), 1.0e-30)
        )
        constraint_error = float(
            action_error.norm() / max(direct.norm(), 1.0e-30)
        )
...
def build_endpoint_trace_mass_actions(
    ...
    mesh_data: Any,
    constraints: TraceConstraintMap,
    selections: tuple[EndpointTraceMassSelection, EndpointTraceMassSelection],
) -> tuple[EndpointTraceMassAction, EndpointTraceMassAction]:
    """Assemble one two-tag full form and return two sparse endpoint actions."""
    ...
    integrand = ufl.inner(
        ufl.cross(normal, trial), ufl.cross(normal, test)
    )
    ...
    try:
        for offset, selection in enumerate(selections, start=1):
            actions.append(_build_one_action(full, selection, constraints, offset))
