...
class P6CellCondensedAction:
    ...
    def buffer_inventory(self) -> Mapping[str, Any]:
        ...
        for cell in self._cells:
            retained_arrays.extend(
                [
                    cell.S_V,
                    cell.recovery,
                    cell.trace_from_interior,
                    cell.expansion,
                    cell.active_ids,
                    cell.original_trace,
                    cell.original_interiors,
                    cell.ports,
                    cell.Bi,
                    cell.Bt,
                    cell.Di,
        ...
        for rows, values in (*self._direct_B_original.values(), *self._direct_D_original.values(),
            ...
            port_arrays.extend([rows, values])

        def unique_nbytes(values: Sequence[Any]) -> int:
            seen: set[int] = set()
            total = 0
            for value in values:
        ...
        return MappingProxyType(
            {
                ...
                "unique_cache_bytes": unique_nbytes([*retained_arrays, *port_arrays]),
            }
        )
