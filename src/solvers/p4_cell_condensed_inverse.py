...
class P4RefinementLedger:
    """Return only cumulatively refined p4 states.

    ``P4CellCondensedInverse.apply`` is intentionally a one-RHS operation and
    therefore its ``last_port_solution`` describes that RHS only.  A
    refinement must not expose that delta as the total port state.  This
    ledger keeps the original RHS, the accumulated FE correction, and the
    accumulated appended-port vector together.  ``apply_a4`` receives both
    total objects so the caller cannot accidentally evaluate a delta-port
    state as if it were the current solution.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(inverse, P4CellCondensedInverse):
            raise TypeError("P4RefinementLedger requires a P4CellCondensedInverse")
        if not callable(apply_a4):
            raise TypeError("apply_a4 must be callable")
        if (
            float(tolerance) <= 0.0
            or int(max_refinements) < 0
            or int(max_refinements) > 2
        ):
        ...
        if self.tolerance > 1.0e-10:
            raise ValueError("p4 refinement tolerance may not exceed 1e-10")
        self.failure_sink = failure_sink
        if not callable(port_closure):
            raise TypeError("P4RefinementLedger requires an independent port_closure callback")
        self.port_closure = port_closure
        self.logical_apply_calls = 0
        self.last_audit: dict[str, Any] = {}
        self.total_port_solution = np.zeros(
            int(inverse.condensed.appended_rows), dtype=np.complex128
        )

    def _factor_counts(self) -> dict[str, int]:
        factor = self.inverse.factor
        return {
            "symbolic_calls": int(getattr(factor, "symbolic_calls", 0)),
            "numeric_calls": int(getattr(factor, "numeric_calls", 0)),
            "solve_calls": int(getattr(factor, "solve_calls", self.inverse.solve_count)),
            "logical_apply_calls": int(self.logical_apply_calls),
        }

    def _port_facts(self) -> dict[str, Any]:
        value = np.ascontiguousarray(self.total_port_solution, dtype=np.complex128)
        return {
            "size": int(value.size),
            "norm": float(np.linalg.norm(value)),
            "sha256": hashlib.sha256(value.tobytes()).hexdigest(),
            "policy": "cumulative_total_not_last_delta",
        }

    def _port_closure_facts(
        self,
        total: PETSc.Vec,
    ) -> dict[str, Any]:
        """Check the accumulated carrier equation independently of A4(c)."""

        try:
            facts = dict(self.port_closure(total, self.total_port_solution))
            status = str(facts.get("status", "P4_NUMERICAL_UNQUALIFIED"))
            if status == "PASS":
                relative = float(facts.get("relative_residual", np.inf))
                finite = bool(facts.get("finite", False))
                if not finite or not np.isfinite(relative) or relative > self.tolerance:
    ...
    def _failure(
        self,
        rhs: PETSc.Vec,
        total: PETSc.Vec | None,
        residual: PETSc.Vec | None,
        ...
        *,
        ...
        facts = {
            ...
            "port_state": self._port_facts(),
            "logical_wall_seconds": float(
                perf_counter() - getattr(self, "_logical_started", perf_counter())
            ),
        }
        self.last_audit = facts
        if self.failure_sink is None:
            return
        packet = {
            "status": facts["status"],
            "matrix_identity": facts["matrix_identity"],
            "factor_counts": facts["factor_counts"],
            "port_state": np.asarray(self.total_port_solution).copy(),
            "g": np.asarray(rhs.getArray(readonly=True)).copy(),
            "c": None if total is None else np.asarray(total.getArray(readonly=True)).copy(),
            "r": None if residual is None else np.asarray(residual.getArray(readonly=True)).copy(),
            "rows": rows,
        }
        try:
            self.failure_sink(packet)
        except BaseException as sink_error:
            self.last_audit["failure_sink_error"] = {
                "type": type(sink_error).__name__,
                "error": str(sink_error),
            }

    def solve(self, rhs: PETSc.Vec) -> PETSc.Vec:
        ...
        self.total_port_solution.fill(0.0)
        try:
            if rhs.getSize() != self.inverse.condensed.full_rows:
                raise ValueError("p4 refinement RHS has the wrong storage size")
            rhs_values = np.asarray(rhs.getArray(readonly=True))
            if not np.isfinite(rhs_values).all():
                raise ValueError("p4 refinement RHS is non-finite")
            rhs_norm = float(rhs.norm())
            if not np.isfinite(rhs_norm):
                raise FloatingPointError("p4 refinement RHS norm is non-finite")
            # One logical call represents one solve(g) entry.  Refinements
            # reuse that logical call and add only physical MatSolve work.
            self.logical_apply_calls += 1
            if rhs_norm == 0.0:
                total = rhs.duplicate()
                ...
                self.last_audit = {
                    ...
                    "factor_counts": self._factor_counts(),
                    "port_state": self._port_facts(),
                    "logical_wall_seconds": float(perf_counter() - self._logical_started),
                }
                return total
            ...
            for refinement in range(self.max_refinements + 1):
                ...
                row = {
                    ...
                    "rhs_norm": rhs_norm,
                    "factor_counts": self._factor_counts(),
                    "attempt": int(refinement + 1),
                    "refinement_index": int(refinement),
                    "physical_factor_delta": (
                        initial_factor_delta if refinement == 0 else {
                            "symbolic_calls": 0,
                            "numeric_calls": 0,
                            "solve_calls": 0,
                        }
                ...
                if port_facts.get("status") != "PASS":
                    error = FloatingPointError(
                        "accumulated p4 port closure is not qualified"
                    )
                    self._failure(rhs, total, residual, rows, error=error)
                    failure_recorded = True
                    residual.destroy()
                    residual = None
                    raise error
                if not np.isfinite(relative):
                    error = FloatingPointError("p4 refinement residual is non-finite")
                    self._failure(rhs, total, residual, rows, error=error)
                    failure_recorded = True
                    residual.destroy()
                    residual = None
                ...
                if refinement == self.max_refinements:
                    error = RuntimeError("p4 A4 residual exceeded the qualified limit")
                    self._failure(rhs, total, residual, rows, error=error)
                    failure_recorded = True
                    residual.destroy()
                    residual = None
                ...
                try:
                    ...
                    residual = None
                after = self._factor_counts()
                row["physical_correction_factor_delta"] = self._factor_delta(before, after)
                try:
                    total.axpy(PETSc.ScalarType(1.0), correction)
                    delta_port = np.asarray(
                        self.inverse.last_port_solution, dtype=np.complex128
                    )
                    if delta_port.shape != self.total_port_solution.shape:
                        raise ValueError("p4 refinement port state has the wrong size")
                    if not np.isfinite(delta_port).all():
                        raise FloatingPointError("p4 refinement port correction is non-finite")
                    self.total_port_solution += delta_port
                finally:
                    correction.destroy()
