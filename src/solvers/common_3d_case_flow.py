...
def run_prepared_3d_case_flow(
    ...
    *,
    ...
    elif solve_stage4_dtn_port and not (
        dtn_result is not None
        and dtn_result.get("solver_info", {}).get("matrix_free_dtn_component_only")
    ):
        E_incident_solution = incident_air_plane_wave_field(E.function_space, cfg)
        ...
        _write_progress_event(
            ...
            extra={
                "lifecycle_note": (
                    ...
                    if external_solver_snapshot
                    else "Telemetry-only baseline preserves the Task28 lifecycle: "
    ...
    summary = {
        ...
        **_summary_base_fields(cfg, comm),
        ...
        "matrix_free_dtn_component_only": (
            None
            if dtn_solver_info is None
            else dtn_solver_info.get("matrix_free_dtn_component_only")
        ),
        "matrix_free_dtn_probe": (
            None
            if dtn_solver_info is None
        ...
        summary.update(
            {
                ...
                "external_residual_limit": dtn_solver_info["residual_limit"],
                "external_no_global_factor": dtn_solver_info["no_global_factor"],
                "external_solver_profile": dtn_solver_info["solver_profile"],
                "external_assembled_matrix_released_before_solve": (
                    dtn_solver_info["assembled_matrix_released_before_solve"]
                ),
    ...
    if not official_result:
        ...
        if assemble_only_result:
            ...
            log("WARNING: external solver did not converge; official RTA was not run.")
