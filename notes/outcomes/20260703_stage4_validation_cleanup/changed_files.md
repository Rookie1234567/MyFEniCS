...
- `src/common/config_3d.py`：增加 EUV 13.5 nm、Si 光栅复折射率、材料标签和验证角色字段。
- `src/main.py`：PyCharm 3D Stage 4 输入改为 Si 光栅复折射率，并传递材料标签和验证角色。
- `src/postprocessing/diffraction_3d.py`：统一 probe 后处理官方 R/T 来源为 E/H Fourier，E-only 和 net flux 标为诊断。
- `src/runners/run_3d_cases.py`：Stage 4 默认光栅材料改为 Si 复折射率，CLI 增加材料标签和验证角色。
- `src/runners/run_3d_airbox_old.py`：旧 3D 入口同步材料标签和验证角色，避免误读。
- `src/solvers/common_3d_case_flow.py`：运行日志头部打印材料、波长和验证角色。
...
- `src/test/stage2_test_utils.py`：Stage 4 测试默认光栅材料改为 Si 复折射率。
- `src/test/stage4_2p5d_compare.py`：旧 2.5D 对照标记为数值 sanity。
- `src/test/test_11_stage4_diffraction_modes.py`：增加官方 R/T 来源单元测试。
- `src/test/test_13_3d_stage_entrypoints.py`：增加 PyCharm 3D 参数中的 Si 光栅复折射率检查。
