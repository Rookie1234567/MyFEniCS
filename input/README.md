...
| `solver.coarse_degree` | `integer` | `polynomial degree` | no | `—` | 2, 3, 4 | full3d_iterative | V25 direct physical coarse degree | `coarse_degree` | V25 only; the selected Q2/Q3/Q4 stage fixes the actual value | `4` |
| `solver.physical_operator_backend` | `enum` | `none` | no | `—` | native_profile_default, isotropic_sum_factorized_n1e_v26 | full3d_iterative | V25 physical volume-action backend identity | `physical operator backend` | older profiles keep their native default | `"native_profile_default"` |
| `solver.h6_backend_rule` | `enum` | `none` | no | `—` | native_ffcx_power10_then_packed_apply, isotropic_sum_factorized_n1e_v26_apply_and_power10, direct_selected_backend_same_apply_and_power10 | full3d_iterative | H6 apply and power-estimation backend rule | `H6 backend rule` | explicit V25/V26 backend contract | `"native_ffcx_power10_then_packed_apply"` |
| `solver.thread_contract` | `enum` | `none` | no | `—` | profile_default, mpi1_omp1_blas1_v25, mpi1_omp1_blas1_v26 | full3d_iterative | explicit execution-thread contract | `thread contract` | V25/V26 selected profile fixes MPI1/OMP1/BLAS1 | `"profile_default"` |
| `solver.numeric_cache_mode` | `enum` | `none` | no | `—` | build | full3d_iterative | 跨进程局部数值 packet 的策略 | `numeric cache mode` | V26 正式场必须从头 build；禁止加载既有局部数值 packet | `"build"` |
...
<!-- schema-field {"key":"solver.coarse_degree","unit":"polynomial degree","applicability":["full3d_iterative"]} -->
<!-- schema-field {"key":"solver.physical_operator_backend","unit":"none","applicability":["full3d_iterative"]} -->
<!-- schema-field {"key":"solver.h6_backend_rule","unit":"none","applicability":["full3d_iterative"]} -->
<!-- schema-field {"key":"solver.thread_contract","unit":"none","applicability":["full3d_iterative"]} -->
<!-- schema-field {"key":"solver.numeric_cache_mode","unit":"none","applicability":["full3d_iterative"]} -->
