# Case060：Task030 低内存 H(curl) 迭代研究

## 当前状态

Case060 当前为 `workstation_success_experimental_opt_in`。五类真正 p/h 候选在 h5 100 步均为明确负结果；最终有效候选保留 Task27 的 75D Floquet z-hat coarse，并使用 symmetric pre/post ILU0、subdomain-local shift、factor-only storage 和 FGMRES restart90。h5/h3/h2 full solve 均已通过；h2 为 1873 步、真残差 `9.972228e-7`、含 R/T/A 峰值 9.374729 GB。
...
| 2. 证明 | H(curl) transfer/Galerkin 正确性与 compact low-memory PC 的 h5/h3 工程证据 |
...
| 21. records | 只提交轻量 hierarchy/transfer/screen/h5/h3 摘要 |
...
最终 compact candidate：h5 855 步、`9.924905e-7`、1.696136 GB；h3 962 步、`9.903890e-7`、3.807503 GB。两者 R/T/A 对 direct 最大差小于 `5.44e-9`。h3 相对 canonical 5.082275 GB 的内存降幅为 25.08%，h3/h5 iteration ratio 1.1251。
