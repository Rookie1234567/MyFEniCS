...
Case070 的最终分类是 `strong_memory_success_slow_but_memory_efficient`。在不改变冻结物理、80 个 DtN 模态、exact condensation 或 ordinary default 的前提下，h2 MPI4 full solve 的 external simultaneous worker RSS 峰值为 7.897675 GiB，真实 full residual 为 `9.998454e-7`，official R/T/A 与 direct 最大差 `6.126e-9`，且无 swap。相对 Task030 历史 9.374729 GiB 的辅助观察降幅约 15.8%；Task31 legacy internal peak 为 8.176441 GiB，对照时约降 12.8%，故保守工程结论是从约 9.4 GiB 压缩到约 8.0–8.2 GiB。
...
| 2. 证明 | 冻结 target 上 assembled-F-free public MPC form action、较小 overlap 与 compact lifecycle 可在真实收敛下把 h2 external simultaneous RSS 压到 8 GiB 以下 |
...
| 10. operator | exact `F-C H^-1D`；solve 中 fine `F` 使用 assembled-F-free public MPC form action；不是缓存优化的低层 element kernel |
| 11. MPI | 4 ranks |
...
| 15. storage | factor-only、subdomain-local shift、assembled-F-free fine action、compact lifecycle |
...
| mesh | FE DoF | iterations | full residual | simultaneous worker peak | vs Task030 historical（辅助） | solve / total (s) |
|---|---:|---:|---:|---:|---:|---:|
...
h3 同时满足绝对 `<=3.50 GiB` 和任务书的历史基线相对 `>=8%` Gate。h2 的 central predictions 为 8.501/8.587 GiB，保守上界 9.447 GiB；external 实测 7.898 GiB 低于两者，并达到 strong memory success。Task030/Task31 sampler 并非完全同实现，因此表中百分比只作辅助；绝对 Task31 peak 是主要结论。三份正式 record 都来自同一 tracked-source-clean commit；h2 无 warning、无 termination、swap in/out 都为 0。

## 正确性与结构证据

- public MPC form action 相对 assembled `F` 的误差为 h5 `9.718e-16`、h3 `9.460e-16`、h2 `9.248e-16`；solve ledger 中不再保留 `F`。
- h5 200-step assembled 与 public form-action residual 分别为 `8.611995756e-4` 和 `8.611995763e-4`，差异仅约 `6.3e-13`；form-action screen 时间约 3.18 倍。
...
内存收益来自 solve 阶段不常驻 assembled `F`、较小 overlap factor 与 solver/RTA 对象解耦；restart50 和 selective Jacobi 没有形成足够收益。`release_f()` 的一次性销毁不是主要瓶颈，public MPC form action 每次 apply 的 Function/MPC/form assembly/通信才是主要时间代价，因此 Case070 是 memory-first，不是吞吐优化。

## 代码路径与理论

调用链是 `run_task031_memory_forensics -> run_workstation_iterative -> mpc_form_action / condensed_dtn / physical_slab_two_level -> official RTA`。端口合法性与选择规则见 [`../../../docs/iterative_solver_ports.md`](../../../docs/iterative_solver_ports.md)，理论与对象生命周期见 iterative theory 以及 walkthrough 31/32/33/50。
...
最终 profile 的收敛证据只覆盖冻结 RHS/物理/分区。FGMRES 的 flexible PC 不是数学意义上的任意问题“保证收敛”；本 Case 的“保证”是 explicit three-residual full solve Gate。当前 public form-action 路线仍在每次 apply 调用 `assemble_vector(ufl.action(...))`，不是低层缓存 kernel，因而造成明显时间代价，适合内存受限而非追求吞吐的机器。
