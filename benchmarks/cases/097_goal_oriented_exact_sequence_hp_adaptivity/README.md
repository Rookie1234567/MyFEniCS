...
### Selective-master 与历史研究分支

选择性合并后的 master 只保留通用 numerical core、explicit-opt-in
research API、checker、compact records 和文档。下面涉及 candidate-specific
`generate_*` / `analyze_*`、固定 plan 或旧 per-rank record 的完整复现命令，
只适用于远程历史分支：
...
这些 research-only 文件没有删除，也没有提升为 production。master 的可移植
验证入口是本目录 `test_command.txt`：focused serial、MPI8 representative、
registry/compact checker 和 full suite；按用户效率要求不重复 MPI2。若本地
Git 对象库仍含历史 source commit，compact contract 会逐文件重算历史 blob
SHA；clean/shallow master clone 没有该对象时，仍验证冻结 commit/file/hash
元数据和 tracked record 自身 SHA。
...
python -m pytest -q \
  ...
  src/test/test_185_task035d_variable_p_petsc_assembly.py \
  src/test/test_186_task035d_variable_p_transfer.py \
  src/test/test_189_task035d_variable_p_reduction.py \
  src/test/test_case097_compact_evidence_contract.py

mpiexec -n 8 python -m pytest -q \
  src/test/test_201_task035d_broken_cell_trace.py \
  src/test/test_205_task035d_unit_channel_adjoint_basis.py
```
