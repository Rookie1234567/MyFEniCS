...
def _read_processes() -> dict[int, dict[str, Any]]:
    ...
    for entry in proc.iterdir():
        ...
        if (
            "--worker" in cmdline
            or "benchmarks.run_workstation_iterative" in cmdline
            or "benchmarks.run_task032_phase6_augmented" in cmdline
        ):
            try:
...
def _sample(root_pid: int, progress_path: Path, elapsed: float) -> dict[str, Any]:
    ...
    row: dict[str, Any] = {
        ...
        "worker_rank_smaps_rollup_json": json.dumps(
            [
                {
                    "rank": item["rank"],
                    "pid": item["pid"],
        ...
        "worker_rank_smaps_readable_count": len(readable_smaps),
