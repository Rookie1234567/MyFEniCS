...
| F3 screen 20（[record](../../../benchmarks/cases/100_static_condensed_full3d_iterative/records/task37_f3_20_screen_v1.json)） | reported/full `0.0302833465991175` | screen-only；RTA not run | `13.2211914063 GiB`，`159.96 s` | screen pass |
| F3 screen 100（[record](../../../benchmarks/cases/100_static_condensed_full3d_iterative/records/task37_f3_100_screen_v1.json)） | reported/full `0.000608485581260`；last-40 ratio `0.1852104694` | screen-only；RTA not run | `12.9641036987 GiB`，`212.73 s` | screen pass |
| F3 screen 200（[record](../../../benchmarks/cases/100_static_condensed_full3d_iterative/records/task37_f3_200_screen_v1.json)） | full `3.5885919793e-5`；predicted `323` it / `473.764 s` | 200-step decision pass；RTA not run | `12.9706878662 GiB`，worker `293.35 s` | authorized screen |
...
factor-only ILU(0)、global direct factor count `0`、global Schur materialized
`false`，但这些结构结果不能替代内存 Gate。
