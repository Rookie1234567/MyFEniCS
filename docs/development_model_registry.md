...
| M6B-W13A | 在冻结 W5 75D ProjectedRangePC 组合中固定比较 beta=1.0 与 beta=0.5 | W5 projected `0.9995565651228495→0.9940090684868385`；W7 projected `0.9999083283541277→0.9937069526556399`；改善 `0.5550%/0.6202%` | process-tree peak `1,717,895,168 B`，`/usr/bin/time` MaxRSS `1,695,490,048 B`，swap `0` | `W13A_DIAGNOSTIC_EXECUTION_COMPLETE`；W13B 固定 5% improvement Gate fail/locked | `m6b_w13a_projected_range_composition.json` |

W12 的预测 live set 是 derived/predicted，不能替代上述实测 process-tree peak。W13A 的 `1,726,081,915 B` 同样是 derived prediction，不是 W13A 的实测峰值。所有阶段的 full time-harmonic PDE、official field/RTA、direct-authority physics comparison 和最终 `<2,000,000,000 B` PDE Gate 仍为 `not_run`。W8–W12 的 source、summary/watchdog 和离线 JSON SHA 见 [`m6b_w8_w12_consolidated_closeout.json`](../benchmarks/cases/101_task37_extra_development/records/m6b_w8_w12_consolidated_closeout.json)；W13A action-only diagnostic 已完成，逐文件 hash-bound 证据见 [`m6b_w13a_projected_range_composition.json`](../benchmarks/cases/101_task37_extra_development/records/m6b_w13a_projected_range_composition.json)。由于两组 residual 的 beta=0.5 改善仅 `0.5550%/0.6202%`，低于固定 5% 门，W13B 200 步 screen 不解锁。
