# Review V12：interlevel route selection v1（截至 C2 closeout）

## 当前结论

R0 的合同、阈值和历史证据仍然冻结。R3 Route B 结构资格通过后，R4.2 已完成一次 p6/h10/MPI1 setup-only 资源与生命周期审计；离线 checker-v2 判为 `SETUP_EVIDENCE_PASS`。R4.3 random 已运行到 checkpoint 7000 后由用户因性能趋势明确受控停止；这不是自然退出、不是 10000 步 numerical Gate 失败，也不改变任何旧负结果。随后 C1.1 只做同一 mesh 的 p3/p1 H(curl) owner/Floquet physical-canonical identity 诊断，local algebra 通过但 MPI1/MPI2 coefficient identity 未通过，因此 C1 已关闭。C2 的小型 MPI1 owner/topology 诊断随后发现 `h3star→h1star` work Gate 失败；按 Review V12 §11.3，C2 与本轮多层 PC 路线现已关闭。

| 项目 | 截至 C2 closeout 的状态 | 边界 |
| --- | --- | --- |
...
| 当前 route_B | `STRUCTURALLY_QUALIFIED_FOR_R4`（历史结构资格） | `selected_hierarchy=NONE`；后续 positive/PC 资格未形成 |
...
| C2 nested LOR-edge HMG | `CLOSED` | `h3star→h1star` owned-packet work `0.018392534459166617 > 1e-11`；无 qualified multilevel PC |
...
| R7 / C2 | `CLOSED` | C2 hard Gate 失败；停止 numerical implementation |
| R8–R11 | `not_run_by_gate` | 没有 selected hierarchy 或 p6 positive qualification |
| R12 | `docs_closeout` | 创建 C2 compact、候选架构比较和 `response_v12.md`；不代表 solver/physics 通过 |
...
`Route B 当时的 structural 状态为 STRUCTURALLY_QUALIFIED_FOR_R4`，表示它满足进入 R4 审阅的结构前置条件；其后 R4.3 random 只到 7000 步并受控停止。当前全局结论是 `selected_hierarchy=NONE`。R4.2 setup 已为 `SETUP_EVIDENCE_PASS / resource-qualified-for-R4.3`，但 random 在 7000 步仍有 `0.00814181052296021` 的显式真残差，用户因此拒绝继续承担预计十万级迭代成本。Route B 不能据此被写成 positive solver 通过；C1.1 随后因 physical canonical MPI identity Gate 关闭，C2 的 MPI1 smoke 又在第二个 nested pair 触发 work Gate，故 `nested_lor_edge_hmg_v1=CLOSED`，R8–R11 为 `not_run_by_gate`，R12 仅为文档收口。
...
当前 `selected_hierarchy=NONE`。R4.3 random 已在 7000 步受控停止，gradient/curl/checkerboard 仍为 pending/not_run；C1.1 已因 MPI physical canonical identity Gate 关闭，C2 也已关闭；R8–R11 继续为 `not_run_by_gate`，不能把 setup resource qualification 写成最终 solver 或 PDE 通过。
...
因此 Route B 目前仍保留其 structural/setup 历史，但不被选择为当前 hierarchy；这次只冻结性能受控停止事实。C1 `same_mesh_hcurl_pmg_v1` 的 owner identity 诊断随后已完成并因 cross-MPI physical canonical coefficient defect 关闭；C2 已按下一节的 MPI1 work Gate 关闭，不能写成通过。
...
### C2：nested LOR-edge HMG 已关闭

C2.0 的纯局部 `h6→h3star→h1star` transfer 通过了固定节点子集、局部伴随和组合审计；C2.1a 只建立了 owner runtime scaffold。随后唯一一次 C2.1b/C2.1c 小型真实 MPI1 `p6/h50` smoke 使用固定 analytic primal（backsubstitution）和固定 slave-zero dual，逐层检查 raw/owner bridge，再检查两个 pair 的全局 work identity。它不是 p6/h10 formal、不是 KSP/V-cycle/PDE，也没有启动 MPI2。

| C2.1c 事实 | 实测值/状态 |
| --- | --- |
| executed source / case / MPI | `f7d0ac41678b2d18be6c05c1eebfde87adcf9521` / `p6-h50` / MPI1 |
| test exit / wall | `1` / `108.41 s`；仅因最终 violation assertion |
| h6 bridge | primal owner roundtrip `0`；dual roundtrip `0`；raw/owner work `3.54641052899015e-16` |
| h3star bridge | primal owner roundtrip `0`；dual roundtrip `0`；raw/owner work `4.096217945654923e-15` |
| h1star bridge | primal owner roundtrip `0`；dual roundtrip `0`；raw/owner work `2.671632823616697e-16` |
| h1star raw primal representation | `0.19459042014393269`；仅为 constrained slave storage 表示差异，owner packet roundtrip 为 `0` |
| h6→h3star pair | owned-packet work `2.176782822433302e-15`；raw PETSc work `6.2401107576421675e-15` |
| h3star→h1star pair | owned-packet work `0.018392534459166617 > 1e-11`；raw PETSc work `0.048176780898148176` |
| resource scope | rank-worker max RSS `486473728 B`；rank swap `0 B`；不是完整 process-tree qualification |
| violations | 仅 `h3star->h1star transfer work` |

三个 level 的 owner inventories 与 bridge work 都闭合，第一对 transfer 也闭合；因此现有事实只能把失败隔离到 `h3star` 作为 fine level 时的 `h3star→h1star` owner-packet work。它不能在不再运行或构造反事实的情况下唯一归因于 incidence、orientation、phase、ordering 或 dual-route 中某一条 production 公式，故没有猜修。按 Review V12 §11.3，`nested_lor_edge_hmg_v1=CLOSED`，并且 V12 不再保留 qualified multilevel PC。

诊断 record 为 [`nested_lor_edge_hmg_c2_mpi1_diagnostic_v1.json`](records/nested_lor_edge_hmg_c2_mpi1_diagnostic_v1.json)。测试文件修改前 SHA256 为 `9266aee14f833670e5d981730734d8f5ba1b41d4509bec75a99244f65650c533`；随后只加入默认 skip、显式 `TASK038_RUN_C2_1B_REAL=1` 才执行原始 smoke 的 archive guard，最终文件 SHA 为 `c02130b6e6df744d9e58df97c5cf2452f3f5e098fd657d699a2b76bb97983c93`，未改 source 或 algebra。两份仍存在的临时 capture 仅覆盖第一对和 MPI1：`primal_mpi1.json` SHA `bd26e754e68872f36e6e22bffcb523fe17e88e6b9d30672da94318016046d8ec`（9,278,943 B，21,732 packets），`dual_mpi1.json` SHA `060b097322d0e7d5c8b9d4c5f9eea4a8ba72ec140e2b909d1a5f68c68652c114`（1,059,538 B，2,538 packets）。它们不是跨 MPI authority；原始 stdout 没有单独持久化。

| C2 选择性整合边界 | 决定 |
| --- | --- |
| C2.0 local nested transfer | `research-only retained local oracle/infrastructure`；不提升为 production PC |
| C2.1a/b runtime 与 `test326` | `research-only / do-not-merge candidate evidence`；保留负结果，不接入 ordinary default |
| `nested_lor_edge_hmg_v1` 整体 | `CLOSED`；没有 qualified multilevel PC |
| selected hierarchy | `NONE` |
| C2.2、p6/h10、physical Maxwell、official physics | `not_run_by_gate` |
