...
```text
stage4_flat_layer_sanity = 平坦界面 sanity / flat-layer 校准 case
stage4_block_grating    = 3D 周期矩形柱/光栅散射路径
```

不要把 small-cell flat-layer 写成“Stage 4A 3D 光栅散射”。small-cell flat-layer 只是为了校准端口、体吸收和后处理口径的平坦界面 sanity case。真实 3D 光栅/矩形柱散射应使用 `stage4_block_grating`。

---

## 2. 背景与当前判断

task003 已经基本修复了 flat-layer 中的主功率闭合问题。当前判断是：
...
本轮任务不再强制回到真实 100 nm grating。当前目标是把 small-cell flat-layer 固化为标准 sanity benchmark，并研究：

1. p=1 和 p=2 的收敛性；
2. p=2 是否能让 `probe_eh_fourier` / `net_flux` 更接近 `port`；
3. MPI 并行 1/4/8 进程是否改变数值结果；
4. 当前分支改动是否破坏 Stage 1 / Stage 2 / Stage 4 相关功能。
...
```text
建立一个小电脑可运行的 small-cell flat-layer 收敛与并行一致性验证链条。
```
...
5. 在 small-cell 收敛与 MPI 检查通过后，运行 Stage 1 / Stage 2 / `stage4_flat_layer_sanity` / `stage4_block_grating` 的轻量回归。

---

## 4. small-cell flat-layer 标准设置
...
该设置是 flat air/Si interface，不是 grating。由于没有横向结构，理论上只应存在零级传播通道。高阶衍射级不应被激发。
...
### 7.2 `stage4_block_grating` zero-contrast smoke test

`stage4_block_grating` zero-contrast 可以使用小尺寸或低成本配置，目标是验证：

```text
3D grating 几何、材料 tag、输出结构、port/volume/probe/net_flux 路径没有崩溃。
```

zero-contrast smoke test 不应写成真实 grating 物理 benchmark。
...
n_substrate = 1 + 0j
n_grating = 1 + 0j
stage4_dtn_order_policy = zero_order 或低成本 auto_propagating
```
...
MPI 4/8 与串行是否一致？
Stage 1/2/flat-layer sanity/stage4_block_grating smoke 是否仍能通过轻量回归？
...
7. Stage 1 / Stage 2 / flat-layer sanity / stage4_block_grating zero-contrast 的轻量回归完成。
...
- 不要把 small-cell flat-layer 称为“Stage 4A 3D 光栅散射”。
- `stage4_flat_layer_sanity` 是平坦界面 sanity。
- `stage4_block_grating` 才是 3D 周期矩形柱/光栅散射路径。
- 不要用 `R+T≈1` 单独判断正确性。
