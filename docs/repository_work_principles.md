...
13. **failed solver code 默认留在对应 research branch，不合并 production。** 文档、review、精简 outcomes 和理论笔记可以选择性合并。
14. 禁止整体合并大型 research branch；必须从 clean base 通过 `selective_merge_manifest` 抽取经过验证、可维护的最小组件。
15. ordinary solver default 不得静默改变。新的 direct、iterative、condensed 或 workstation profile 在完成审查前必须保持显式 opt-in。
16. 未通过最终 review 之前，不建议合并到 `master`；最终状态必须明确为 `pass`、`pass_with_qualifications` 或 `fail`。
17. 若审查后发现问题，可在同一任务分支持续修改和复审；只有新增物理功能、新求解算法、大规模参数研究、ordinary default 变更或超范围架构重写才拆分为新任务。

## 5. 数值可信度

18. solver 成功必须使用 full explicit true residual 判断，不能只使用 preconditioned/KSP residual、相对零解 improvement 或内部 projected residual。
19. official R/T/A 只能从通过 residual Gate 的场计算；probe-plane Fourier、sampled flux 和其他近似量只能标记为 diagnostic，除非独立任务重新证明其 official 资格。
20. direct/iterative、auxiliary/condensed、MPI1/MPI4 或缓存复用结果必须有明确等价性、R/T/A、能量闭合、内存和环境证据；不得把未运行项写成通过。
21. 失败结果同样必须保留准确结论，禁止把研究正信号包装成 production 能力，也禁止因后续文档精简而删除关键负结果边界。
