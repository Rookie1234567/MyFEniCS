...
1. 为 public MPC matrix-free action 设计可缓存/批量 apply，减少每次 form action 的装配与通信开销；
...
在这些证据出现前，Task030 是速度较优但约 9.375 GiB 的 profile；Task031 是内存较优但约 5.01x solve time 的 profile。应按机器约束选择，而不是替换 ordinary default。
