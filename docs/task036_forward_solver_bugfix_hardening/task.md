...
base_branch = master
working_branch = codex/20260730-task36-forward-solver-bugfix-hardening
...
本分支必须直接基于最新远程 master；本轮实际起始基线为：

```text
origin/master = 007298261681014efbe6508ac91c6c3ae9a6a44a
```

Task036 文档仅从提交
`f0f10be9268ca9348ed7f40fb2237fbcd5474252` 恢复本目录。不得整体 merge
其他分支，不得整体 cherry-pick Task035e 或 surrogate 分支，不得直接提交或
合并 master。所有提交只推送到与 working branch 同名的远程分支。
...
同时必须阅读当前 Task036 分支（直接源自最新 master）中与共享求解器相关的
代码和测试，至少包括：
...
- 提交并推送到
  `origin/codex/20260730-task36-forward-solver-bugfix-hardening`；
