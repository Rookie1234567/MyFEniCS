...
同 scope 的三场正式时间对照如下：完整 workflow 使用各自 `run_summary`，纯 KSP 使用 physical summary 的 `solver.ksp_solve_monotonic_seconds`，不把 `monitor.solve_seconds` 当成纯 KSP；RSS 是 watchdog 同时进程树峰值。差值分钟以 r2 为基准。
...
本次已有 `formal_release_timing` 还记录了：A6 live `263` 次、累计 `823.1639212121663 s`（volume `822.071849550819 s`、DtN `0.7940302900387906 s`），native A6 witness `29` 次、`148.35112204798497 s`；H6 apply `131` 次、`484.89414036102244 s`，matrix-mult `282` 次、`518.9228249350854 s`，power10 `20` 次、`36.88009328895714 s`。这些 action clocks 可能嵌套重叠，不能与完整 workflow 相加。
...
另有一场 02:50 的旧 `swap_observe` 尝试：source=`e82e6cfc1a774e384f1c1cc98d41f7838d5d51ca`，raw root 为 `.../20260922T025001.832290Z`，分类是 `USER_CONTROLLED_STOP`，full workflow monotonic=`2041.675088969001 s`，最后外层 A6 checkpoint 24 残差=`0.001188840920712491`。它不是 global-swap stop，费用不在本记录中清零。
