...
|---|---|---|
| 完整PQ固定g定位，450255f4575792d052c1bac29837d39955ee1039 | CUg场误差0.930671134/0.930658530；左反馈delta/y仅0.000239758/0.000241966 | 61项raw核验通过，仅固定方向诊断；父55.683027095s，RSS783474688B。此前dd9b工程序列化失败原样保留 |
| 首次完整B4_T(g)，564e42b43391f1857934ef064778636aff06894b | 真残差2.876315279；场误差0.862153759/0.861991284 | 单次作用未收敛；按预定合同继续唯一有界I4，没有用单次rho>1跳过检查 |
| 同source唯一I4 | 22步真残差0.134111120，未达1e-4；场误差**0.040714718/0.040705987** | INNER_INEXACT_AT_CAP，场误差较旧I4约0.997显著降低，是后续依据，不关闭实体路线 |

I4的16步真残差0.168839973；22步完整回调显式61.549079180s，末次显式64.912397272s。原60s限制未延长、未重启。cold全流程保守115.004649472s，全树RSS采样峰883875840B，最低系统可用11504521216B；tree/global swap增量0，父和23个后代全部退出。11430项独立raw核验通过，包括所有实体volume+DtN、局部LU、F/FH、CUg桥、W/Q平衡、真实A4残差与场度量；raw审计12.227573395s另计，使用保存LU做3960列三角重构，无新因子/FE/I4。

共23次B4_T、47次通用CU/S logical、47次MatSolve、refine0；A4共76次，耗时33.264126437s（均值0.437685874s）。既有缓存volume forward71次耗时3.500925499s，adjoint24次耗时1.576964705s。嵌套的CU/H_T费用不与内部volume费用再次相加。

只读用原class/map/80port比较reference、first B4和I4终点三个向量：缓存体积+原端口对native A4相对差4.5244e-12/3.4318e-13/4.4724e-12，均<1e-11，离线数组作用约0.046s。这不是全空间误差界或新I4速度实测，但支持一项不改数学的热点优化。现仅实现待短审：Krylov/BAL用现成cell_volume+原streaming DtN；每16步和退出的显式真残差仍由native A4独立计算。新入口需重核三向量及完整same-g B4输出，再唯一零初值I4，600s cold与60s/64步/1e-4、安全Gate全部不变；尚未正式运行。无新outer或official R/T/A，G5仍开放。
