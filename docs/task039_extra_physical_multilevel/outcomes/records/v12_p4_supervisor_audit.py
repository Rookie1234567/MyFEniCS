...
records = root / 'benchmarks/artifacts/task39extra/v12_o0_o4/e8c3c82bab2687a811a11f0798a2879c725532b5/o1_full_physical_controls_0/records'
errors, rows = [], []
checks = 0
...
result = {'scope': 'Saved vector arithmetic and cell-energy recount only; no new operator application, no independent FE quadrature, no outer-PC PASS', 'source': 'e8c3c82bab2687a811a11f0798a2879c725532b5', 'records': str(records), 'completed_p4_records': len(rows), 'completed_I4_calls_lower_bound': len(rows), 'completed_B4_calls_lower_bound': 5*len(rows), 'completed_MD_local_solves_lower_bound': 210*len(rows), 'checks': checks, 'errors': errors, 'status': 'SAVED_RECORDS_CONSISTENT' if not errors else 'INCONSISTENT', 'rows': rows}
Path('/tmp/task39extra-v12-p4-supervisor-audit.json').write_text(json.dumps(result, indent=2) + '\n')
