...
- full repository pytest：每个 Phase 最多一次，只在解锁下一 Phase 或最终交付前运行。
...
完成后可直接进入 **Phase C-low-cost estimator bake-off**，无需等待新的 review，也无需立即运行 full repository pytest。
...
B3/B4 必须在正式选择 Phase D mesh backend 或运行任何 p4/h5 adaptive heavy case 前完成或形成明确 controlled-negative 决定。
