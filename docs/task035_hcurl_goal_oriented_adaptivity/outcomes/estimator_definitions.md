...
最低 Gate 已通过，解锁 Phase C-low-cost；B3/B4 仍待完成。
...
| material interface precursor | 手工 coefficient-aware jump/recovery | material tag、naive recovery | `algebraic_precursor_pass`；B3 pending |
| Hybrid precursor | 手工 Et/Ht 与 error split | Et/Ht trace | `algebraic_precursor_pass`；B4 pending |
...
- B3/B4 仍为 `pending`，R4 维持 `formula_defined`；
- Phase C-low-cost 已开始，但尚无 target-grating screen、adaptive cycle、真实 p4、
  heavy PDE 或 mesh backend 结论。
