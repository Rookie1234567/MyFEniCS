...
checker 修复提交为 `d64398cb1fecd90867071688dca94e501235cf7a`，只让显式 `None` 时间门限跳过比较并保留 iteration Gate；4 项 focused tests 通过。独立纯读盘 recheck 的 tracked 副本见 [`5nm_checker_recheck.json`](outcomes/records/5nm_checker_recheck.json)，来源为 ignored `recheck/checker.json`，结果 `independent_output_gates_passed=true`、`gate_failures=[]`、`BALANCED_OUTPUT_AUTHORITY_LIMITED`。完整 600 条 mode key、复振幅、边界振幅及逐通道功率见 [`5nm_dtn_600_channels.csv`](outcomes/records/5nm_dtn_600_channels.csv)，compact 中保留来源 SHA。`REFERENCE_AUTHORITY_LIMITED` 仅表示没有短波 matched fine/continuum 精度参考，不是缺少必需的 A4 数值求解。
