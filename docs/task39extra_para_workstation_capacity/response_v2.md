...
只读速度证据显示，R2 已记录的3次 PC 中 `A_structure/C/smoother` 平均为 `67.3023/17.8440/1.0281 s`，R1 前32次同字段为 `9.27665/8.66633/1.46391 s`；总 PC 操作约 `4.44x`，R2 step1→3 平均约 `5.13x` R1 前32平均 outer-step。p4 日志为 `native=6`、refinement=`0`、BAL_H，mode bridge=`FIELDWISE_MODE_IDENTITY_PASS`，支持已走 qualified native 路径但不能单独确定根因。终止后8秒只读基线为CPU23 `1.0 GHz`、热区约`67–68/53–54°C`、运行线程`0/19`，不代表运行期频率或热限；不据此归因、不自动重跑。
