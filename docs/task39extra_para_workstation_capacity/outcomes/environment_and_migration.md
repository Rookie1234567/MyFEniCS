...
用户已在BMC网页设为Full，反馈CPU2周围内存仍热；没有把该反馈当成温度恢复证据，CPU2降频未宣布解决。后续用户允许读取硬件，非交互sudo读取本项目perf数据可用；早期“sudo不可用”仅是当时状态。没有自行改风扇模式、PROCHOT或全局CPU参数。

最终运行绑定为worker CPU23、parent CPU9；前述CPU8/24安排仅为早期历史。R1 retry1前后clean source均为b2e132a7b1f1078eb3359c87a336123b3c7dfbdd，完整监督及清场通过，但性能screen未通过。新增native FFCx循环调度与PSS降频为显式opt-in，普通V5数学和默认路径不变；最小身份异常分类修复不更改安全阈值。性能代码与测试详情见[测试报告](test_summary.md)。历史场数组缺失并未通过运行native reference补齐：reference尚未解锁，不能把早期准备安排写成已经完成。
