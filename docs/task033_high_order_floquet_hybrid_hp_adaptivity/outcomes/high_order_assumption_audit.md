...
| A24 | variable-p cellwise H(curl) has no qualified native path in the audited runtime | DOLFINx 0.10.0.post2 / Basix 0.10.0 mixed/submesh APIs do not prove unequal-p conformity, periodic synchronization or MPI ownership | out-of-scope | fail-closed audit complete; no microfixture triggered | only prototype if native sparse orientation-safe support is demonstrated; otherwise retain fixed-p+h route | `records/variable_p_capability_audit.json` |
